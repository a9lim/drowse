"""Tests for the native /drowse/v1/* API (no GPU)."""
...
from drowse.server.ws_models import WSSamplingParams, build_sampling_config
...
class TestSessions:
    def test_list(self, session_and_client: Any) -> None:
        ...
        with patch("drowse.server.session_models.supports_thinking", return_value=False):
            resp = client.get("/drowse/v1/sessions")
    ...
    def test_create_idempotent(self, session_and_client: Any) -> None:
        _, client = session_and_client
        with patch("drowse.server.session_models.supports_thinking", return_value=False):
            resp = client.post("/drowse/v1/sessions", json={})
        assert resp.status_code == 200
    ...
    def test_create_model_mismatch_logs_warning(self, session_and_client: Any, caplog: Any) -> None:
        _, client = session_and_client
        with patch("drowse.server.session_models.supports_thinking", return_value=False):
            resp = client.post("/drowse/v1/sessions", json={"model": "other/model"})
        assert resp.status_code == 200
    ...
    def test_safe_model_id_is_not_a_session_alias(
        ...
        _, client = session_and_client
        with patch("drowse.server.session_models.supports_thinking", return_value=False):
            resp = client.get("/drowse/v1/sessions/test__model")
        assert resp.status_code == 404

    def test_get_not_found(self, session_and_client: Any) -> None:
        _, client = session_and_client
        resp = client.get("/drowse/v1/sessions/other")
        assert resp.status_code == 404

    def test_delete_is_noop(self, session_and_client: Any) -> None:
        _, client = session_and_client
        resp = client.delete("/drowse/v1/sessions/default")
    ...
    def test_patch_top_k_distinguishes_omitted_from_explicit_null(
        ...
        with patch("drowse.server.session_models.supports_thinking", return_value=False):
            preserved = client.patch(
                "/drowse/v1/sessions/default", json={"temperature": 0.4},
            )
            cleared = client.patch(
                "/drowse/v1/sessions/default", json={"top_k": None},
            )
    ...
    def test_validate_steering_parses_and_dry_installs(
        ...
        # real ``~/.drowse`` made the outcome depend on whether that home
        # happened to hold current-format bundled folders.
        import drowse.io.selectors as selectors
        ...
        session, client = session_and_client
        resp = client.post(
            "/drowse/v1/sessions/default/steering/validate",
    ...
    def test_validate_steering_returns_expected_errors_in_band(
        ...
        session, client = session_and_client
        session.steering.side_effect = DrowseError(
    ...
    def test_clear(self, session_and_client: Any) -> None:
        session, client = session_and_client
        resp = client.post("/drowse/v1/sessions/default/clear")
        assert resp.status_code == 204
    ...
    def test_rewind_empty(self, session_and_client: Any) -> None:
        session, client = session_and_client
        session.tree.messages_for.return_value = []
        resp = client.post("/drowse/v1/sessions/default/rewind")
        assert resp.status_code == 400
...
class TestProfiles:
    def test_list_empty(self, session_and_client: Any) -> None:
        _, client = session_and_client
        resp = client.get("/drowse/v1/sessions/default/profiles")
        assert resp.status_code == 200
        assert resp.json()["profiles"] == []

    def test_get_not_found(self, session_and_client: Any) -> None:
        _, client = session_and_client
        resp = client.get("/drowse/v1/sessions/default/profiles/missing")
        assert resp.status_code == 404

    def test_delete_not_found(self, session_and_client: Any) -> None:
        _, client = session_and_client
        resp = client.delete("/drowse/v1/sessions/default/profiles/missing")
        assert resp.status_code == 404
...
class TestProbes:
    ...
    def test_defaults(self, session_and_client: Any) -> None:
        _, client = session_and_client
        with patch(
            "drowse.server.probe_routes.load_default_manifolds",
            return_value={"emotion": ["happiness"]},
        ):
            resp = client.get("/drowse/v1/sessions/default/probes/defaults")
        assert resp.status_code == 200
    ...
    def test_attach(self, session_and_client: Any) -> None:
        ...
        resp = client.post(
            "/drowse/v1/sessions/default/probes", json={"selector": "happy"},
        )
    ...
    def test_attach_empty_selector(self, session_and_client: Any) -> None:
        _, client = session_and_client
        resp = client.post(
            "/drowse/v1/sessions/default/probes", json={"selector": "  "},
        )
        assert resp.status_code == 400

    def test_deactivate_not_found(self, session_and_client: Any) -> None:
        _, client = session_and_client
        resp = client.delete("/drowse/v1/sessions/default/probes/missing")
        assert resp.status_code == 404
...
class TestExtract:
    def test_extract_json(self, session_and_client: Any) -> None:
        ...
        resp = client.post(
            "/drowse/v1/sessions/default/extract",
    ...
    def test_raw_profile_load_route_is_removed(
        ...
        session, client = session_and_client
        resp = client.post(
            "/drowse/v1/sessions/default/profiles",
...
class TestWebSocket:
    ...
    def test_generate_worker_typed_error_uses_safe_message(
        ...
        session, client = session_and_client
        session.generate.side_effect = DrowseError("safe generation detail")
    ...
    def test_session_mismatch_closes(self, session_and_client: Any) -> None:
        ...
        with pytest.raises(Exception):
            with client.websocket_connect("/drowse/v1/sessions/other/stream") as ws:
                ws.receive_json()
    ...
    def test_ws_requires_bearer_when_api_key_set(self):
        ...
        with client.websocket_connect(
            "/drowse/v1/sessions/default/stream?token=s3cret",
        ) as ws:
            ws.send_json({"type": "frobnicate"})
            msg = ws.receive_json()
            assert msg["type"] == "error"
...
class TestDeletedTraitsStream:
    ...
    OpenAI/Ollama ``x-drowse-probe-readings`` extension are the live per-token
    delivery channels."""
    ...
    def test_traits_stream_route_is_not_registered(
        ...
        _, client = session_and_client
        resp = client.get("/drowse/v1/sessions/default/traits/stream")
        assert resp.status_code == 404
    ...
    def test_session_has_no_trait_queue_plumbing(self) -> None:
        ...
        for attr in (
            ...
            assert not hasattr(DrowseSession, attr)
...
class TestManifoldRoutes:
    def test_create_list_get(self, session_and_client: Any, tmp_path: Any, monkeypatch: Any) -> None:
        ...
        resp = client.post("/drowse/v1/manifolds", json=_box1d_payload())
        assert resp.status_code == 201
        body = resp.json()
        ...
        listed = client.get("/drowse/v1/manifolds").json()["manifolds"]
        ...
        detail = client.get("/drowse/v1/manifolds/local/mood").json()
    ...
    def test_create_conflict(self, session_and_client: Any, tmp_path: Any, monkeypatch: Any) -> None:
        monkeypatch.setenv("DROWSE_HOME", str(tmp_path))
        _session, client = session_and_client
        assert client.post("/drowse/v1/manifolds",
    ...
    def test_create_manifold_from_template(
        ...
        assert client.post(
            "/drowse/v1/templates", json=template_payload,
        ).status_code == 201
        payload = {
            "name": "calendar",
            "fit_mode": "auto",
        ...
        resp = client.post("/drowse/v1/manifolds/from-template", json=payload)
        ...
        weekday_row = next(
            m for m in client.get("/drowse/v1/manifolds").json()["manifolds"]
            if m["name"] == "calendar"
        )
        assert weekday_row["resolved_fit_mode"] is None
        assert weekday_row["domain_label"] == "discover-auto"

        detail = client.get("/drowse/v1/manifolds/local/calendar").json()
        monday = next(n for n in detail["nodes"] if n["label"] == "monday")
        assert monday["statements"] == ["today is Monday", "it's Monday"]

        conflict = client.post("/drowse/v1/manifolds/from-template", json=payload)
        assert conflict.status_code == 409
        forced = client.post(
            "/drowse/v1/manifolds/from-template",
    ...
    def test_create_manifold_from_template_missing_template(
        ...
        monkeypatch.setenv("DROWSE_HOME", str(tmp_path))
        _session, client = session_and_client
        resp = client.post("/drowse/v1/manifolds/from-template", json={
            "name": "bad",
    ...
    def test_create_manifold_from_template_ambiguous_template(
        ...
        assert client.post(
            "/drowse/v1/templates", json={**template, "namespace": "local"},
        ).status_code == 201
        assert client.post(
            "/drowse/v1/templates", json={**template, "namespace": "other"},
        ).status_code == 201
        resp = client.post("/drowse/v1/manifolds/from-template", json={
            "name": "calendar",
    ...
    def test_patch_description(self, session_and_client: Any, tmp_path: Any,
        ...
        monkeypatch.setenv("DROWSE_HOME", str(tmp_path))
        _session, client = session_and_client
        client.post("/drowse/v1/manifolds", json=_box1d_payload())
        resp = client.patch("/drowse/v1/manifolds/local/mood",
    ...
    def test_delete(self, session_and_client: Any, tmp_path: Any, monkeypatch: Any) -> None:
        ...
        assert client.delete(
            "/drowse/v1/manifolds/local/mood").status_code == 200
        assert client.get(
            "/drowse/v1/manifolds/local/mood").status_code == 404

    def test_get_missing(self, session_and_client: Any, tmp_path: Any, monkeypatch: Any) -> None:
        monkeypatch.setenv("DROWSE_HOME", str(tmp_path))
        _session, client = session_and_client
        assert client.get(
            "/drowse/v1/manifolds/local/ghost").status_code == 404
    ...
    def test_authored_node_label_resolves_without_restart(
        ...
        ``drowse serve`` is long-lived, so a resolver index warmed before the
        ...
        assert client.post(
            "/drowse/v1/manifolds", json=_box1d_payload(),
        ).status_code == 201

        hit = selectors.resolve_manifold_label("afraid")
        assert hit is not None and hit.manifold_key == "local/mood"

        assert client.delete("/drowse/v1/manifolds/local/mood").status_code == 200
    ...
    def test_delete_refuses_when_busy(self, session_and_client: Any, tmp_path: Any,
        ...
        assert client.delete(
            "/drowse/v1/manifolds/local/mood").status_code == 409
    ...
    def test_fit_json(self, session_and_client: Any, tmp_path: Any, monkeypatch: Any) -> None:
        ...
        resp = client.post(
            "/drowse/v1/manifolds/local/mood/fit", json={"layers": [1, 2]},
        )
        assert resp.status_code == 200
        body = resp.json()
    ...
    def test_fit_authoring_conflict_is_a_retryable_409(
        ...
        resp = client.post("/drowse/v1/manifolds/local/mood/fit", json={})
        assert resp.status_code == 409
    ...
    def test_fit_sse_authoring_conflict_is_a_typed_frame(
        ...
        with client.stream(
            "POST",
            "/drowse/v1/manifolds/local/mood/fit",
            json={},
            headers={"accept": "text/event-stream"},
        ) as resp:
            text = b"".join(resp.iter_bytes()).decode("utf-8")
...
class TestTemplateRoutes:
    def test_create_list_get_delete(
        ...
        monkeypatch.setenv("DROWSE_HOME", str(tmp_path))
        _session, client = session_and_client
        resp = client.post("/drowse/v1/templates", json=_TMPL_PAYLOAD)
        assert resp.status_code == 201, resp.text
        body = resp.json()
        ...
        listing = client.get("/drowse/v1/templates").json()["templates"]
        assert any(t["name"] == "weekday" for t in listing)

        detail = client.get("/drowse/v1/templates/local/weekday").json()
        assert detail["slot"] == "[DAY]"
        assert len(detail["contexts"]) == 2
        assert detail["contexts"][1]["turns"][-1]["content"] == "remind me the day?"

        assert client.delete("/drowse/v1/templates/local/weekday").status_code == 200
        assert client.get("/drowse/v1/templates/local/weekday").status_code == 404
    ...
    def test_create_slot_in_history_rejected(
        ...
        assert client.post("/drowse/v1/templates", json=bad).status_code == 400
    ...
    def test_score_route_wires_session(
        ...
        monkeypatch.setenv("DROWSE_HOME", str(tmp_path))
        session, client = session_and_client
        client.post("/drowse/v1/templates", json=_TMPL_PAYLOAD)

        from drowse.core.scoring import ChoiceScore, ChoiceScores
        ...
        resp = client.post(
            "/drowse/v1/templates/local/weekday/score",
    ...
    def test_score_missing_template_404(
        ...
        _session, client = session_and_client
        assert client.post(
            "/drowse/v1/templates/local/ghost/score", json={},
        ).status_code == 404
...
class TestPairwiseMetric:
    ...
    def test_mahalanobis_default(self, session_and_client: Any) -> None:
        ...
        session.whitener = w
        r = client.get("/drowse/v1/sessions/default/profiles/pairwise?a=x&b=y")
...
class TestAnalyticsMultiNodeProbe:
    ...
    def _wire(self, session_and_client: Any) -> tuple[Any, TestClient]:
        ...
        session.analytics_names = lambda: DrowseSession.analytics_names(session)
        session._live_direction_tensors = (
            lambda n: DrowseSession._live_direction_tensors(session, n)
        )
        session.analytics_profile = (
            lambda n: DrowseSession.analytics_profile(session, n)
        )
    ...
    def test_correlation_explicit_multinode_404(self, session_and_client: Any) -> None:
        _, client = self._wire(session_and_client)
        r = client.get("/drowse/v1/sessions/default/correlation?names=fan,vx")
        assert r.status_code == 404

    def test_pairwise_multinode_404(self, session_and_client: Any) -> None:
        _, client = self._wire(session_and_client)
        r = client.get("/drowse/v1/sessions/default/profiles/pairwise?a=fan&b=vx")
        assert r.status_code == 404
...
class TestNativeErrorEnvelope:
    """Every ``/drowse/v1/*`` failure renders as ``{"detail": "<string>"}``.

    The three shapes a native route can fail in — a typed ``DrowseError``, a
    ...
    def test_bare_http_exception_detail_is_a_string(
        ...
        _, client = session_and_client
        resp = client.get("/drowse/v1/sessions/nope")
        assert resp.status_code == 404
        body = resp.json()
        assert set(body) == {"detail"}
        assert isinstance(body["detail"], str)

    def test_typed_drowse_error_uses_the_native_envelope(
        self, session_and_client: Any,
    ) -> None:
        from drowse.core.errors import DrowseError

        class _Nope(RuntimeError, DrowseError):
        ...
        resp = client.post(
            "/drowse/v1/sessions/default/instruments/geometry/live",
            json={"enabled": True},
        )
    ...
    def test_protocol_error_renders_each_envelope(self) -> None:
        ...
        from drowse.server.app import _protocol_error

        native = _protocol_error("/drowse/v1/sessions", 409, "busy")
