...
The safetensors save/load for a fitted :class:`~drowse.core.manifold.Manifold`
...
``io`` importing ``core``'s dataclasses (:class:`~drowse.core.manifold.Manifold`,
:class:`~drowse.core.manifold.LayerSubspace`, :class:`~drowse.core.manifold.ManifoldDomain` via :func:`~drowse.core.manifold.domain_from_spec`) is the
...
:mod:`drowse.io.manifold_folder`; this module owns just the fitted-tensor pair
...
class ActivationRowStore:
    ...
    def __init__(self, node_sizes: Sequence[int]) -> None:
        ...
        self._tmp: tempfile.TemporaryDirectory[str] | None = (
            tempfile.TemporaryDirectory(prefix="drowse-manifold-rows-")
        )
...
def save_manifold(
    ...
    **Affine (flat / folded-vector) layers** write ``layer_<L>.mean`` +
    ...
    # Every drowse safetensor artifact is stored fp32 (cast at the writer);
    ...
    if (
        ...
        if sidecar.get("fit_policy_version") != MANIFOLD_FIT_POLICY_VERSION:
            raise ManifoldFormatError(
                ...
                "policy; refit it with the current drowse"
            )
