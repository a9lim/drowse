"""Standalone templated-completion artifact — ``~/.drowse/templates/<ns>/<name>/``.
...
from drowse.io.paths import ensure_within, templates_dir
...
class TemplateFormatError(ValueError, DrowseError):
...
def template_lock_path(path: Path) -> Path:
    path = Path(path)
    return path.parent / f"{path.name}.template"


@contextmanager
def template_lock(path: Path):
    with artifact_lock(template_lock_path(path)):
        yield
...
class AmbiguousTemplateError(ValueError, DrowseError):
    """A bare template name resolves to more than one namespace."""


class TemplateNotFoundError(KeyError, DrowseError):
...
# artifact kind.  Bundled templates live under ``drowse/data/templates/<name>/``
# in the wheel and materialize into ``~/.drowse/templates/default/<name>/`` on
...
# exactly one place — :func:`drowse.io.bootstrap.materialize_bundled_artifacts`,
...
def bundled_template_names() -> list[str]:
    """List valid templates shipped under ``drowse/data/templates/``."""
    try:
        root = _resources.files("drowse.data.templates")
...
def materialize_bundled_templates() -> None:
    """Copy bundled templates into ``~/.drowse/templates/default/``.

    Mirrors :func:`drowse.io.manifolds.materialize_bundled_manifolds` for the
    ...
    Process-scoped no-op after the first call for a given ``DROWSE_HOME``.
    Switching homes in-process bootstraps the new root.  Must run BEFORE
    ``materialize_bundled_manifolds``; call
    :func:`drowse.io.bootstrap.materialize_bundled_artifacts`, which owns that
