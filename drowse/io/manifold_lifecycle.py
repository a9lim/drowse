...
it stays physically in :mod:`drowse.io.manifolds` so the process-scope
...
from drowse.core.errors import ManifoldExistsError, ManifoldNotFoundError, DrowseError
...
def _manifold_tensor_files(
    ...
    """
    from drowse.io.paths import parse_tensor_filename, safe_model_id
...
def _capture_group_is_referenced(safe_model: str, capture_sha: str) -> bool:
    ...
    from drowse.io.paths import manifolds_dir, parse_tensor_filename
...
def _remove_unreferenced_capture_groups(
    ...
    from drowse.io.paths import models_dir
...
def _refresh_manifold_locked(
    ...
    - ``hf://<owner>/<name>[@rev]`` — re-pulled into the same folder via
      :func:`drowse.io.hf_manifolds.pull_manifold`; returns ``"hf"``.
    ...
    *bypassed*: delete just that model's fitted tensor pair (via
    ...
    if source.startswith("hf://"):
        ...
        from drowse.io.hf_manifolds import pull_manifold
...
def _proven_neutral_loaded_fingerprint(
    ...
    """
    from drowse.io.alignment import validate_neutral_cache_metadata
...
def preflight_manifold_fit_noop(
    ...
    *,
    ...
    from drowse.core.model import config_model_shape, model_source_fingerprint
...
def _transfer_manifold_locked(
    ...
    *,
    ...
    (the shape :func:`drowse.io.alignment.fit_alignment` produces), and writes a
    ...
    :func:`drowse.io.paths.tensor_filename`'s transfer suffix).
    ...
    :func:`drowse.core.manifold.transfer_manifold_subspaces`; this function only
    ...
    **Target-metric re-bake (mandatory).**  The fitted manifold's
    ...
    raises :class:`~drowse.core.mahalanobis.WhitenerError` — there is no
    ...
    """
    from drowse.core.manifold import transfer_manifold_subspaces
    ...
    # ``WhitenerError`` (a ``DrowseError``) on a missing / partial target
    ...
    # format-error handling is unchanged.  The ``DrowseError`` guard runs first
...
def manifold_summary(
    ...
    ``GET /drowse/v1/manifolds/{ns}/{name}`` (server) — the keys both can
