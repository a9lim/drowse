...
from :mod:`drowse.io.manifold_folder`; cross-model transfer and lifecycle
live in :mod:`drowse.io.manifold_lifecycle` (not corpus authoring).
"""
...
from drowse.io.paths import manifold_dir, manifolds_dir
...
# ===================================================== discovery + authoring ===
#
# The functions below are the shared backend for `drowse manifold`
# (CLI) and the `/drowse/v1/manifolds` HTTP routes — folder discovery and
...
def iter_manifold_folders(
    ...
    Walks ``~/.drowse/manifolds/<ns>/<name>/``; malformed folders are
...
def create_manifold_folder(
    ...
    try:
        from drowse.core.manifold import normalize_domain_spec
...
def create_manifold_from_template(
    ...
    *,
    ...
    :class:`drowse.io.templates.TemplateFolder` selector), expands its
...
def create_discover_manifold_folder(
    ...
    *,
    ...
    (:func:`drowse.core.manifold.discover_coords` runs over the per-node
    ...
    :func:`drowse.io.manifold_lifecycle.transfer_manifold` — it reuses the
    same per-layer Procrustes map (:mod:`drowse.io.alignment`) the
...
def create_baked_manifold_folder(
    ...
    *,
    ...
    :class:`drowse.core.manifold.Manifold` (typically the affine ``R = 1`` ray
    from :func:`drowse.core.capture.fold_directions_to_subspace`); its geometry
...
def _recoverable_baked_first_publication(
    ...
    *,
    ...
    from drowse.io.manifold_folder import manifold_folder_tensor_paths
    from drowse.io.integrity import verify_integrity
    from drowse.io.paths import parse_tensor_filename, tensor_filename
...
def save_baked_manifold_tensor(
    ...
    *,
    ...
    if method == "merge":
        from drowse.io.manifold_folder import MERGE_BAKE_POLICY
...
def merge_discover_manifolds(
    ...
    *,
    ...
    The vector-side counterpart is :func:`drowse.io.bake.merge_into_manifold`,
    ...
    *node corpora* and lets the next fit derive coords from the
    ...
    materialized. Run ``drowse manifold fit <name> -m <model>`` or
