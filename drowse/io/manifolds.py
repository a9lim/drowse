...
moved out, the names re-export here, so ``from drowse.io.manifolds import X``
keeps working unchanged everywhere:

- :mod:`drowse.io.manifold_folder` — the on-disk format core: the
  dataclasses, integrity/sha256, ``manifold.json`` load/save, and the
  shared module-private validators + payload builders.
- :mod:`drowse.io.manifold_tensors` — the fitted per-model tensor codec
  (``save_manifold`` / ``load_manifold``) and the activation-row spool.
- :mod:`drowse.io.manifold_authoring` — folder discovery and the
  create/init/append/plan/merge/update authoring path.
- :mod:`drowse.io.manifold_lifecycle` — lifecycle (rm/clear/refresh),
...
``drowse.io.manifolds._materialized_home`` to ``None`` directly — so keeping
...
:func:`drowse.io.bootstrap.materialize_bundled_artifacts` is the entry point
...
from drowse.io.bootstrap import canonical_payload_sha256
from drowse.io.paths import manifolds_dir, drowse_home
...
from drowse.io.manifold_lifecycle import (
    ManifoldFitProof,
    TransferSourceProof,
...
_log = logging.getLogger("drowse.io.manifolds")

# Process-scope guard: the ``DROWSE_HOME`` the first
...
# intentionally switch ``DROWSE_HOME`` in-process, and that new root still
...
# reset it via ``drowse.io.manifolds._materialized_home`` (see the module
# docstring).
...
# ``drowse/data/manifolds/<name>/`` in the wheel and materialize into
# ``~/.drowse/manifolds/default/<name>/`` on session startup.  JSON-only
# on the shipped side — per-model ``.safetensors`` fits are produced on
# the user's machine via ``drowse manifold fit``.
...
def bundled_manifold_names() -> list[str]:
    """List complete manifolds shipped under ``drowse/data/manifolds/``."""
    try:
        root = _resources.files("drowse.data.manifolds")
...
def _materialize_one_bundled_manifold(default_dir: Path, name: str) -> None:
    ...
    pkg_root = _resources.files("drowse.data.manifolds").joinpath(name)
...
def materialize_bundled_manifolds() -> None:
    """Copy bundled manifolds into ``~/.drowse/manifolds/default/``.

    For each complete ``drowse/data/manifolds/<name>/`` in the wheel, ensure
    ``~/.drowse/manifolds/default/<name>/`` is current. It only touches
    ...
    (``drowse manifold generate ...`` or hand-author under
    ...
    **Per-home process-scoped no-op after first call.**  Subsequent calls within
    the same process and for the same ``DROWSE_HOME`` return immediately without touching disk.  This
    prevents a second materialize (from e.g. ``DrowseSession.from_pretrained``
    ...
    Use :func:`drowse.io.bootstrap.materialize_bundled_artifacts` rather than
