...
both :mod:`drowse.io.manifold_authoring` and
:mod:`drowse.io.manifold_lifecycle` import from here.
...
:mod:`drowse.core.manifold`).  Manifolds live under their own root,
``~/.drowse/manifolds/<ns>/<name>/``. A manifold carries N labeled nodes on a
domain; a two-node flat manifold is the bipolar-concept case.

```
~/.drowse/manifolds/<ns>/<name>/
...
``drowse manifold fit`` produces the per-model tensors and
back-fills the ``files`` integrity manifest.  Tensor save/load itself
lives in :mod:`drowse.io.manifold_tensors` (``save_manifold`` / ``load_manifold``);
...
from drowse.core.manifold import BoxDomain, domain_from_spec
from drowse.core.role_templates import _ROLE_SLUG_RE
from drowse.io.atomic import write_json_atomic
from drowse.io.integrity import NAME_REGEX, hash_file, verify_integrity
...
def _locked_manifest(folder: Path):
    ...
    from drowse.io.atomic import artifact_lock
...
def manifold_pair_lock_path(tensor_path: Path) -> Path:
    ...
    return tensor_path.parent.parent / f".drowse-pair-{digest}"
...
def domain_label(spec: dict[str, Any]) -> str:
    ...
    kind = spec.get("type", "?")
    if kind == "klein":
        return "Klein bottle (2D)"
    if kind == "projective":
        return "Projective plane / RP² (2D)"
    if kind == "box":
        n = len(spec.get("axes", []))
    elif kind == "sphere":
...
def sanitize_hyperparams(
    ...
    for key, value in hyperparams.items():
        ...
        if not valid:
            raise ManifoldFormatError(
                f"fit_mode {fit_mode!r} hyperparameter {key!r} has invalid "
                f"value {value!r}"
            )
...
# without loading weights (:func:`drowse.io.manifold_lifecycle.
...
MANIFOLD_SIDECAR_FIELDS = {
    "format_version", "name", "method", "drowse_version", "domain",
    ...
    "capture_version", "context_binding_sha256", "capture_render_sha256",
...
MANIFOLD_METHOD_FIT_MODES: dict[str, frozenset[str]] = {
    ...
    # Older monopolar sidecars inherited the writer's authored default because
    # the early-return fit branch omitted its source-manifest provenance.
    # Current writers correctly retain the one-node discover-pca identity.
    "manifold_monopolar": frozenset({"authored", "pca"}),
    "manifold_monopolar_sae": frozenset({"authored", "pca"}),
...
def validate_manifold_sidecar_payload(
    ...
    from drowse.io.brand_migration import LEGACY_NAMES, migrate_legacy_record

    legacy_brand = isinstance(data, dict) and any(
        f"{name}_version" in data for name in LEGACY_NAMES
    )
    data = migrate_legacy_record(data)
    if (
        legacy_brand
        and set(data) == MANIFOLD_SIDECAR_FIELDS - {"context_binding_sha256"}
        and data["fit_mode"] == "baked"
    ):
        # Original baked artifacts cannot be refitted. An absent context-binding
        # proof means unknown, never a verified context binding.
        data["context_binding_sha256"] = None
    ...
    from drowse.core.manifold import validate_domain_spec

    validate_domain_spec(data["domain"])
    for key in ("name", "method", "drowse_version", "feature_space", "fit_mode"):
    ...
    if any(
        ...
        for key in (
            "context_binding_sha256", "capture_render_sha256",
            "baseline_prompts_sha256",
        )
    ):
...
def canonical_manifold_sidecar_payload(
    *, name: str, method: str, drowse_version: str, domain: dict[str, Any],
    ...
    payload = {
        ...
        "name": name, "method": method, "drowse_version": drowse_version,
        ...
        "context_binding_sha256": None, "capture_render_sha256": None,
...
def _validate_node_role(name: str, label: str, role: Any) -> str | None:
    ...
    :data:`drowse.core.role_templates._ROLE_SLUG_RE`
    (``[a-z0-9._-]+``).  Family-unsupported (Mistral-3) is *not*
    checked here — the folder is model-agnostic; the check fires when
    :func:`drowse.core.role_templates.apply_with_role` runs at fit time.
    """
    if role is None:
        return None
...
class ManifoldFormatError(ValueError, DrowseError):
...
def validate_manifold_format_version(
    ...
    if (
        ...
        raise ManifoldFormatError(
            ...
            "with the current drowse."
        )
    return value


class BakedManifoldError(ValueError, DrowseError):
...
def min_nodes(n: int) -> int:
    ...
    embedding), which :func:`drowse.core.manifold.fit_rbf_interpolant`
    enforces.
    """
    return 2 * n + 1
...
class ManifoldSidecar:
    ...
    # (:func:`drowse.core.model.model_source_fingerprint`): resolved Hub commit
    ...
    # capture-identity schema revision (:data:`drowse.core.extraction.
    ...
    # :func:`drowse.io.bake.merge_into_manifold`.  ``None`` on every fit that
    ...
    def load(cls, path: Path) -> "ManifoldSidecar":
        ...
        return cls(
            method=data["method"],
            drowse_version=data["drowse_version"],
            domain=data["domain"],
...
def fitted_sidecar_is_current(path: Path) -> bool:
    """Whether a fitted sidecar carries the current schema generation.
    ...
    Early v10 writers predated the nullable ``context_binding_sha256`` proof.
    That one exact predecessor field set is also stale: it is safe to refit from
    the corpus, but it cannot prove that its capture used the current context.
    Other same-version schema damage remains corruption and reaches the strict
    loader below.
    """
    try:
    ...
    payload = migrate_legacy_record(payload)
    legacy_v10_fields = MANIFOLD_SIDECAR_FIELDS - {"context_binding_sha256"}
    return set(payload) != legacy_v10_fields
...
class ManifoldFolder:
    ...
    loaded through :func:`drowse.io.manifold_tensors.load_manifold`.
    ...
    - ``fit_mode in {"pca", "spectral"}`` (discover mode): nodes carry
      ...
      :func:`drowse.core.manifold.discover_coords` (per-model, since
    ...
    # ``"bundled"`` — shipped under ``drowse/data/manifolds/`` and
    # materialized into ``default/`` (set by namespace at refresh time,
    # not stored); ``"hf://<owner>/<name>[@rev]"`` — pulled from the HF
    # hub (stamped by :func:`drowse.io.hf_manifolds.pull_manifold`).  The
    ...
    # (``drowse.io.templates.TemplateFolder``, ``<ns>/<name>`` or bare name).
        ...
        if fit_mode == "authored":
            ...
            for entry in nodes:
                ...
                if (
                    ...
                    or len(coords) != n
                    or not all(_finite_number(c) for c in coords)
                ):
                    raise ManifoldFormatError(
            ...
            hyperparams = sanitize_hyperparams(fit_mode, raw_hyperparams)
            for entry in nodes:
                if not isinstance(entry, dict):
                    raise ManifoldFormatError(
...
class DiscoverGenerationPlan:
    ...
    Returned by :func:`drowse.io.manifold_authoring.plan_discover_generation`.
