"""Filesystem path helpers for the ~/.drowse/ tree.

All paths resolve through drowse_home(), which honors the DROWSE_HOME
...
def drowse_home() -> Path:
    """Return the root ~/.drowse/ directory. Honors $DROWSE_HOME override."""
    override = os.environ.get("DROWSE_HOME") or os.environ.get("POLYTHETIC_HOME") or os.environ.get("SAKLAS_HOME")
    if override:
        return Path(override)
    from drowse.io.brand_migration import migrate_legacy_home

    return migrate_legacy_home(Path.home())


def models_dir() -> Path:
    return drowse_home() / "models"
...
def ensure_within(root: Path, *parts: str) -> Path:
    ...
    or an absolute component must not be allowed to escape the ``~/.drowse/``
    subtree.
...
def neutral_statements_path() -> Path:
    return drowse_home() / "neutral_statements.json"
...
def baseline_prompts_path() -> Path:
    ...
    Falls back to the bundled ``drowse/data/baseline_prompts.json`` when absent.
    """
    return drowse_home() / "baseline_prompts.json"
