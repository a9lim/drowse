...
All Drowse-owned state lives under `~/.drowse/` and resolves through
`paths.drowse_home()`.
...
`drowse_home()` honors `$DROWSE_HOME`. Helpers: `manifolds_dir`, `manifold_dir`,
...
falling back to bundled `drowse/data/baseline_prompts.json`).
...
writes. That schema is exactly `{format_version, drowse_version, method,
...
`~/.drowse/manifolds/<ns>/<name>/` holds `manifold.json`, `nodes/NN_<label>.json`
...
`from drowse.io.manifolds import X` surface.
...
`validate_manifold_sidecar_payload` enforces the exact schema by set equality.
The sole predecessor adapter accepts an original Saklas/Polythetic v10 baked
sidecar missing only `context_binding_sha256`, treating that browser proof as
unknown (`None`) without rewriting the artifact. A baked direction has no corpus
to refit; non-baked predecessor fits instead remain readable as cache misses,
including under legacy version keys. `method` is a closed discriminator that
`MANIFOLD_METHOD_FIT_MODES` couples to `fit_mode` (`manifold_pca` /
`manifold_sae` → authored; `manifold_monopolar{,_sae}` → legacy authored
or source-preserving PCA;
...
`~/.drowse/templates/<ns>/<name>/template.json`, a peer of a manifold.
...
`lens.py` owns the Drowse-fitted per-model Jacobian lens:
...
the Hugging Face cache without copying them into `DROWSE_HOME`
...
`hf_manifolds.py` is manifold distribution: the `drowse-manifold` tag on a
...
by model scope and variant; the card carries `library_name: drowse`, the
`drowse-manifold` tags, deduped `base_model:`, and `base_model_relation: adapter`.
...
HF pull, local-folder copy, or verified `.drowse` install, with
`ProgressCallback` narrating five stages
...
## drowse_archive.py / gguf_io.py

`.drowse` is the portable manifold transport envelope. It contains exactly
one unchanged manifold v10 closure and its one referenced template when
present. Import rejects unsafe ZIP paths and metadata, verifies declared sizes
and SHA-256 values while streaming, validates fitted safetensors, and promotes
the staged manifold/template pair through a recoverable journaled transaction.
Checksums establish archive integrity, not publisher identity.

`export_drowse_archive` is the general manifold interchange export. The GGUF codec
below remains the specialized folded control-vector export.

The GGUF codec plus its specialized manifold export driver.
`write_gguf_profile(profile, path, *, model_hint)` /
...
`--control-vector-scaled` scalar reproduces drowse's layer weighting with no extra
