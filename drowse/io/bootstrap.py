...
Drowse ships two artifact kinds inside the wheel — templates under
``drowse/data/templates/`` and manifolds under ``drowse/data/manifolds/`` —
which copy-on-miss into ``~/.drowse/`` the first time a process needs them.
Both materializers are process-scoped per ``DROWSE_HOME``, and the **order
...
def materialize_bundled_artifacts() -> None:
    ...
    """
    from drowse.io.drowse_archive import recover_drowse_archive_transactions
    from drowse.io.manifolds import materialize_bundled_manifolds
    from drowse.io.templates import materialize_bundled_templates
