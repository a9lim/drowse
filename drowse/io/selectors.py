...
This module lives under ``drowse.io`` because both the io lifecycle layer and
...
from drowse.io.paths import VARIANT_SUFFIX_RE, manifolds_dir

# Single source of truth lives in ``io.paths`` (owns the variant scheme).
_VARIANT_REGEX = VARIANT_SUFFIX_RE


class SelectorError(ValueError, DrowseError):
...
class ResolvedConcept:
    ...
    ``folder`` is the manifold folder (``~/.drowse/manifolds/<ns>/<name>/``);
...
def _manifold_index() -> _ManifoldIndex:
    ...
    from drowse.io.manifolds import ManifoldFormatError, iter_manifold_folders
...
def all_concepts() -> list[ResolvedConcept]:
    ...
    through :func:`~drowse.io.manifolds.iter_manifold_folders` and already
...
def resolve_manifold_label(
    ...
    :class:`~drowse.core.steering_expr.ManifoldTerm`.
