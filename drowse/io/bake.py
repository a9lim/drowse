...
class MergeError(ValueError, DrowseError):
...
def _resolve_component(
    ...
    (:func:`~drowse.core.capture.folded_directions`).  The returned path
...
def _candidate_term_models(term: _MergeTerm) -> set[str]:
    ...
    from drowse.io.paths import manifold_dir, parse_tensor_filename
...
def _merge_into_manifold_locked(
    ...
    *,
    ...
    ``~/.drowse/manifolds/local/<name>/`` — one fitted per-model tensor for
    ...
    ray (:func:`~drowse.core.capture.fold_directions_to_subspace`) and frozen
