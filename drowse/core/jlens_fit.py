"""The Jacobian-lens estimator — the only backward-pass code in drowse.

Fits ``J_l = E[∂h_final/∂h_l]`` per source layer over a text corpus.  It lives
apart from :mod:`drowse.core.jlens` because nothing on a runtime path needs it:
probes, ``jlens/<word>`` steering atoms, the live workspace readout, and the
token-drilldown replay all want the readout math alone, and they import it per
decode step.
...
``Σ_{t'≥t} ∂h_final[t',r]/∂h_l[t]``; source positions are
averaged within each prompt and prompt Jacobians are then summed, preserving
exact equal-prompt weighting even for ragged lengths. The first
``SKIP_FIRST_POSITIONS`` positions (attention sinks) and the final position are
excluded from both cotangents and the source mean. Backends without batched-VJP
coverage fall back to exact unreplicated scalar VJPs; replicated VJPs remain an
explicit reference mode.

The fit builds its own autograd-enabled forward (``torch.enable_grad()`` + a
grad-seeding output hook on the first fitted block) — the ``inference_mode``
capture machinery in ``capture.py`` cannot be reused, because inference tensors
never re-enter autograd. Per-layer grads come from
``torch.autograd.grad(final, sources)`` — NOT ``backward()`` +
``retain_grad()`` (``.grad`` accumulates across the multi-backward loop and
would corrupt the one-hot-cotangent rows) — which also stops the graph walk at
the shallowest requested source layer, so a band-restricted fit never
backprops below its lowest source. A terminal-layer hook captures the target
residual and aborts the rest of the forward before the final norm and
full-vocabulary head. Row blocks are staged in bounded per-layer stripes,
validated after transfer, and committed directly into the CPU fp32 cross-prompt
accumulator; CUDA double-buffers those stripes so D2H for one can overlap the
next backward block. If a later VJP OOMs, the graph is rebuilt at the first
uncommitted row, and prompt-width backoff splits only that group while
preserving its exact committed prefix and every prior microbatch.  The MPS sync
...
def _resolve_vjp_mode(mode: str) -> str:
    env = os.environ.get("DROWSE_JLENS_VJP")
