...
``~/.drowse/models/<id>/`` (the per-layer centering mean is derived from
...
* :meth:`drowse.core.profile.Profile.cosine_similarity` — the whitened
  cosine metric.
* :func:`drowse.core.capture.project_profile` — LEACE-flavored ``|`` / ``~``
  projection.
...
# ``WhitenerError`` is defined in :mod:`drowse.core.errors` (the single home
# for the engine's error taxonomy) and re-exported here so
# ``from drowse.core.mahalanobis import WhitenerError`` keeps working.
from drowse.core.errors import WhitenerError as WhitenerError
...
def _require_fp32_neutral_cache(
    ...
    if any(v.dtype != torch.float32 for v in acts_raw.values()):
        raise WhitenerError(
            ...
            f"non-fp32 (bf16/fp16) store; drowse requires fp32 for "
...
class LayerWhitener:
    ...
    def from_neutral_activations(
        ...
        *,
        ...
        shape produced by :func:`drowse.core.capture.compute_neutral_activations`).
        ``layer_means`` is ``{layer: [D]}`` — typically the cached means
        from :func:`drowse.io.probes_bootstrap.bootstrap_layer_means`.
    ...
    def from_cache(
        ...
        *,
        ...
        ``neutral_activations.json`` from ``~/.drowse/models/<safe_id>/``
        ...
        already in memory (e.g. inside a live ``DrowseSession``).
        """
        from drowse.io.paths import model_dir
        from drowse.io.alignment import load_validated_neutral_cache
        ...
        try:
            ...
            if expected_identity is not None:
                from drowse.io.alignment import neutral_cache_identity
