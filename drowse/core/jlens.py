...
d-vector with the same shape contract as any drowse steering direction, which
...
The **estimator** — the only backward-pass code in drowse — lives in
:mod:`drowse.core.jlens_fit`.  ``fit_jacobian_lens`` remains reachable as
``drowse.core.jlens.fit_jacobian_lens`` through a lazy module ``__getattr__``,
...
class JacobianLensError(RuntimeError, DrowseError):
...
class MultiTokenWordError(ValueError, DrowseError):
...
def __getattr__(name: str) -> Any:
    ...
    ``fit_jacobian_lens`` moved to :mod:`drowse.core.jlens_fit`; resolving it
    on demand keeps ``from drowse.core.jlens import fit_jacobian_lens``
