...
every drowse input surface (Python, YAML, HTTP, CLI). Every surface
...
:func:`drowse.io.selectors.resolve_bare_atom`'s manifold-label tier (a
...
from drowse.core.errors import SteeringExprError
from drowse.core.triggers import Trigger

if TYPE_CHECKING:
    from drowse.core.steering import AlphaEntry, Steering
...
class ManifoldTerm:
    ...
    named :class:`~drowse.core.manifold.Manifold` artifact on scope entry
    ...
    (:func:`~drowse.core.manifold.subspace_inject`).  Stored as a value
...
# ``SteeringExprError`` lives in :mod:`drowse.core.errors`; it is imported
# above and re-exported here so
# ``from drowse.core.steering_expr import SteeringExprError`` keeps working.
...
def _resolve_atom(atom: _Atom) -> str:
    ...
    :func:`~drowse.io.selectors.resolve_bare_atom` ladder instead.
    """
...
def _fold(
    ...
    *,
    ...
    for term in terms:
        ...
        # :func:`~drowse.io.selectors.resolve_bare_atom` owns end to end —
        ...
        if sel.operator is None and not term.ablation:
            ...
            from drowse.io.selectors import resolve_bare_atom
...
def parse_expr(
    ...
    *,
    ...
    ``None``, :func:`drowse.io.selectors.resolve_bare_atom` raises
    :class:`~drowse.io.selectors.AmbiguousSelectorError` if a bare atom
