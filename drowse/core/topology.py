...
model, no IO.  It depends on :mod:`drowse.core.manifold` for the domain
...
def derive_spectral_coords(
    ...
    *,
    ...
    Stays inside drowse's no-scipy rule.  Noisy below roughly 50 nodes
...
#   (b) periodic axes — bounded H1 evidence proposes a chart, followed by
#       MDS/spectral coordinate checks and a mean-only prediction baseline.
#       This is not a classifier for arbitrary topology; H1 can contain
#       sampling holes on spheres and does not determine a surface's type.
#
...
def _validated_topology_distances(distances: torch.Tensor) -> torch.Tensor:
    if distances.ndim != 2 or distances.shape[0] != distances.shape[1] or not len(distances):
        raise ValueError("topology distances must be a non-empty square matrix")
    if not distances.is_floating_point() or not torch.isfinite(distances).all() or bool((distances < 0).any()):
        raise ValueError("topology distances must be finite and non-negative")
    squared = distances.double().square()
    tolerance = 8 * torch.finfo(distances.dtype).eps * float(squared.max())
    if bool(((squared - squared.T).abs() > tolerance).any()):
        raise ValueError("topology distance matrix must be symmetric")
    if bool((squared.diagonal() > tolerance).any()):
        raise ValueError("topology distance diagonal must be zero")
    result = (distances + distances.T) * 0.5
...
def _rips_h1_persistence(
    ...
    *,
    ...
    """H1 birth/death pairs from exact GF(2) boundary-matrix reduction.

    Infinite death means right-censored at the filtration ceiling, not a
    proven essential cycle of the underlying manifold. Finite bars can carry
    real structure too. Only edges and triangles up to eps_max are reduced.
    Exceeding the triangle budget raises instead of truncating away boundaries
    and manufacturing cycles. Reduction uses CPU integer bitsets.
    """
    distances = _validated_topology_distances(distances)
    if not math.isfinite(eps_max) or eps_max < 0 or max_triangles < 0:
        raise ValueError("persistence ceiling and triangle budget must be non-negative and finite")
        ...
        for k in adj[i] & adj[j]:
            if k > j:  # canonical i<j<k → each triangle once
                ...
                if len(triangles) == max_triangles:
                    raise ValueError("persistent-homology triangle budget exceeded; topology is unresolved")
...
def _count_persistent_loops(
    ...
    *,
    ...
    """Count candidate H1 bars in a bounded Rips window, not true Betti numbers.

    The window ends at twice the longest MST edge. Count both censored and
    finite bars with lifetime >= persistence_frac times their birth scale and
    the connectivity scale. Death must also clear an interior connectivity
    margin, excluding short parallel loops before a rectangular grid connects.
    This recovers smaller finite cycles in some anisotropic tori.

    The thresholds are heuristics. Sampling holes on a sphere can pass them;
    H1 alone does not distinguish a circle, cylinder, or Mobius strip. Chart
    validation is mandatory, and unresolved results are not evidence of
    trivial topology. Inputs and compute budgets are validated explicitly.
    """
    distances = _validated_topology_distances(distances)
    if not math.isfinite(persistence_frac) or persistence_frac < 0 or max_dim < 1:
        raise ValueError("persistence fraction must be finite and non-negative; max_dim must be positive")
    K = int(distances.shape[0])
    if K > 128:
        raise ValueError("persistent topology supports at most 128 nodes; topology is unresolved")
    if K < 4:
        return 0
    ...
    for birth, death in pairs:
        # A thin torus's smaller cycle can die inside the window. Death is
        # not evidence that the cycle was noise: evaluate its lifetime too.
        if (death >= (1.0 + 0.5 * persistence_frac) * eps_c
                and (min(death, eps_max) - birth) >= max(threshold, persistence_frac * birth)):
            count += 1
...
def _untangle_periodic_eigenspaces(eigenvalues: torch.Tensor, eigenvectors: torch.Tensor) -> torch.Tensor:
    """Resolve mixed circle factors inside repeated Laplacian eigenspaces.

    On an equal-radius product torus, arbitrary eigensolver rotations mix the
    sine/cosine planes. Constant quadratic forms identify the factor planes;
    pairing arbitrary columns directly can fold the torus onto itself.
    """
    result = eigenvectors.clone()
    start = 0
    while start < len(eigenvalues):
        end = start + 1
        while end < len(eigenvalues) and float(eigenvalues[end]) <= 1.05 * float(eigenvalues[start]):
            end += 1
        width = end - start
        if 4 <= width <= 8 and width % 2 == 0:
            block = eigenvectors[:, start:end].double()
            pairs = [(i, j) for i in range(width) for j in range(i, width)]
            products = torch.stack([block[:, i] * block[:, j] * (1 if i == j else math.sqrt(2))
                                    for i, j in pairs], dim=1)
            products -= products.mean(dim=0)
            values, vectors = torch.linalg.eigh(products.T @ products)
            for column in range(len(values)):
                if float(values[column]) > 1e-4 * float(values[-1]):
                    break
                quadratic = torch.zeros(width, width, dtype=torch.float64)
                for index, (i, j) in enumerate(pairs):
                    quadratic[i, j] = quadratic[j, i] = vectors[index, column] / (1 if i == j else math.sqrt(2))
                spectrum, rotation = torch.linalg.eigh(quadratic)
                spread = float(spectrum[-1] - spectrum[0])
                if spread < 1e-5:
                    continue
                within = max(float(spectrum[i + 1] - spectrum[i]) for i in range(0, width, 2))
                between = min(float(spectrum[i + 1] - spectrum[i]) for i in range(1, width - 1, 2))
                if within < 0.1 * spread and between > 0.1 * spread:
                    result[:, start:end] = (block @ rotation).to(result.dtype)
                    break
        start = end
    return result


def _periodic_neighborhoods_preserved(distances: torch.Tensor, angles: torch.Tensor) -> bool:
    """Reject a periodic chart that glues distant sampled points together.

    This is a necessary local consistency check, not a homeomorphism proof.
    In particular, H1 alone cannot distinguish a circle from a strip.
    """
    K, dimensions = angles.shape
    if not torch.isfinite(angles).all() or K < 7:
        return False
    for axis in angles.T:
        ordered = torch.sort(axis.remainder(2 * math.pi)).values
        gaps = torch.diff(torch.cat((ordered, ordered[:1] + 2 * math.pi)))
        if float(gaps.max()) >= math.pi:
            return False
    source = distances.clone()
    source.fill_diagonal_(math.inf)
    if float(source.min()) <= 1e-7 * float(distances.max()):
        return False
    embedded = torch.cat((angles.cos(), angles.sin()), dim=1).double()
    mapped = torch.cdist(embedded, embedded)
    mapped.fill_diagonal_(math.inf)
    neighbors = min(2 * dimensions, K - 1)
    local_radius = source.sort(dim=1).values[:, neighbors - 1]
    mapped_radius = mapped.sort(dim=1).values[:, neighbors - 1]
    # Include ties: otherwise an eigensolver or row permutation can hide a fold.
    near = mapped <= mapped_radius[:, None] + 1e-6
    false_neighbors = near & (source > 2.0 * local_radius[:, None])
    collapsed = (mapped < 0.2 * math.pi / K ** (1 / dimensions)) & (source > local_radius[:, None])
    false_neighbors |= collapsed
    return float(false_neighbors.any(dim=1).float().mean()) <= 0.1


def _distance_eigensystem(distances: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    squared = distances.double().square()
    gram = -0.5 * (squared - squared.mean(0) - squared.mean(1, keepdim=True) + squared.mean())
...
def _detect_periodic_axes(
    ...
    *,
    ...
    """Propose a periodic chart, or abstain; this is not arbitrary topology detection.

    H1 bars propose the number of axes. A distance-MDS span with constant
    quadratic forms separates linear product-circle factors without depending
    on arbitrary eigensolver rotations. A Laplacian fallback requires
    nonvanishing, approximately circular eigenpairs and rejects harmonics.
    Ambient-rank, phase-coverage, and neighborhood/collapse checks must pass.

    The guarded faint-ring tour fallback is retained, with an ambient-rank
    check to reject lines. It remains heuristic and is not certified topology.
    None means unsupported or insufficient evidence, not that no loop exists.
    Spheres and nonorientable surfaces are not identified by this routine.
    """
    if len(distances) > 128:
        return None
    d = _count_persistent_loops(
        distances, persistence_frac=persistence_frac, max_dim=max_dim + 1,
    )
    if d > max_dim:
        return None
    if d < 1:
        ...
        if theta is None:
            return None
        values, _ = _distance_eigensystem(distances)
        if int((values > 1e-5 * values[-1]).sum()) < 2:
            return None
        return theta.unsqueeze(1), 1
    values, vectors = _distance_eigensystem(distances)
    # A compact boundaryless d-manifold cannot embed in R^d. Surface holes
    # must not turn a sphere in R^3 into a purported T3/T4 chart.
    if int((values > 1e-5 * values[-1]).sum()) <= d:
        return None
    if 2 * d < len(distances):
        if float(values[-2 * d]) > 1e-7 * float(values[-1]):
            block = vectors[:, -2 * d:]
            block = _untangle_periodic_eigenspaces(torch.ones(2 * d), block)
            coords = torch.atan2(block[:, 1::2], block[:, ::2]).float()
            if _periodic_neighborhoods_preserved(distances, coords):
                return coords, d
    if eigenvalues is not None:
        eigvecs = _untangle_periodic_eigenspaces(eigenvalues, eigvecs)
    ...
    while p * 2 + 1 < avail and len(angles) < d:
        cos_j = eigvecs[:, 2 * p]
        sin_j = eigvecs[:, 2 * p + 1]
        radius_squared = cos_j.square() + sin_j.square()
        mean_radius = float(radius_squared.mean())
        if (mean_radius <= 0 or float(radius_squared.min()) < 0.1 * mean_radius
                or float(radius_squared.std(correction=0)) > 0.35 * mean_radius):
            return None
    ...
    if len(angles) != d:
        return None
    coords = torch.stack(angles, dim=1).contiguous()
    return (coords, d) if _periodic_neighborhoods_preserved(distances, coords) else None
...
def select_topology(
    ...
    **(b) periodic axes** — independently, :func:`_detect_periodic_axes` proposes
    periodic charts from bounded Vietoris–Rips H1 evidence and checked MDS or
    spectral coordinates, with a guarded
    ...
    *topology*, not surface shape — a circle can be linearly embedded yet still
    ...
    candidate that also beats the mean-only GCV baseline routes to a periodic
    ``BoxDomain`` (the curved path). This is not a topology certificate. Spheres are
    **not** auto-selected (a speculative topology that's the least reliable to
    ...
    try:
        ...
        candidates.append(TopologyCandidate(
            "spectral", "spectral", k_flat, math.inf, False, str(e),
        ))
    ...
    try:
        ...
        detected = _detect_periodic_axes(
            ...
            persistence_frac=persistence_frac, eigenvalues=_vals,
        )
        if detected is not None:
            ...
            if (2 * d + 1) <= K:
                ...
                null_score = sum(_gcv_value(float((y - y.mean(0)).square().sum()), 1.0, K)
                                 for y in targets.values())
                viable = math.isfinite(gcv_p) and null_score > 0 and gcv_p <= null_score
                note = (f"Periodic chart candidate; H1 evidence = {n_loops}; not a topology certificate"
                        if viable else "Periodic fit does not improve on the mean-only prediction baseline")
                candidates.append(TopologyCandidate(
                    f"torus-T{d}", "spectral", d, gcv_p, viable, note,
                ))
                if viable:
                    periodic = (p_coords, p_domain, gcv_p, periodic_plan)
    except (ValueError, RuntimeError) as error:
        candidates.append(TopologyCandidate("periodic-unresolved", "spectral", k_flat, math.inf, False, str(error)))

    # Supported periodic charts take priority over a linear approximation: a
    # circle also fits a plane, but the plane does not preserve its closure.
    # Geometry checks and the mean-only baseline above must both pass first.
