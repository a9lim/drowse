...
- :func:`is_static_cache_supported` — the device-agnostic viability probe,
  cached by weak underlying-module identity (through ``torch.compile``'s ``_orig_mod``
...
- :func:`make_static_cache` — the single factory used by
  :mod:`drowse.core.generation`, :mod:`drowse.core.session`, and
  :mod:`drowse.core.model`.  It early-initializes the layer buffers so
...
# Weak model keys prevent support probes and warning state from outliving a
# model after its session closes. They also avoid stale hits if Python reuses
# an object's numeric id for a later model.
_warned_models: WeakSet[Any] = WeakSet()
_support_cache: WeakKeyDictionary[
    Any, dict[tuple[str, torch.dtype | None], tuple[bool, str | None]]
] = WeakKeyDictionary()
...
) -> tuple[Any, tuple[str, torch.dtype | None]]:
...
def is_static_cache_supported(
    ...
    **Device-agnostic** — unlike :func:`is_cuda_graphs_supported`, this does
    ...
    ``(module, device, dtype)`` with a weak module key.
    """
    cache_model, cache_key = _support_cache_key(model, device)
    model_cache = _support_cache.get(cache_model)
    ...
    try:
        ...
        _support_cache.setdefault(cache_model, {})[cache_key] = result
        return result
...
def _patch_static_sliding_mask() -> None:
    ...
    *constant* ``(max_cache_len, 0)`` for every decode step: ``is_full`` stays
    ...
    ``_drowse_static_mask`` flag :func:`make_static_cache` sets only when no slide
    ...
    def get_mask_sizes(self: Any, query_length: int) -> tuple[int, int]:
        if getattr(self, "_drowse_static_mask", False):
            return self.max_cache_len, 0
        return _orig(self, query_length)

    get_mask_sizes._drowse_orig = _orig  # type: ignore[attr-defined]
...
def make_static_cache(
    ...
    for layer in getattr(cache, "layers", []):
        if getattr(layer, "is_sliding", False):
            ...
            if buf is not None:
                layer._drowse_static_mask = max_cache_len <= buf
    return cache
...
def warn_once(model: "PreTrainedModel | torch.nn.Module", reason: str) -> None:
    ...
    """
    key = getattr(model, "_orig_mod", model)
