"""DrowseSession — unified backend for drowse's programmatic and server APIs."""
...
from drowse.core.capture import CaptureMode, CaptureState
...
from drowse.core.naming import BIPOLAR_SEP, _slug, canonical_concept_name
from drowse.core.token_callback import (
    StepTokenCallback,
    TokenCallback,
    TokenConsumer,
...
from drowse.io.probes_bootstrap import bootstrap_layer_means
from drowse.core.profile import Profile, load_profile as _load_profile
...
from drowse.core.steering import SteeringStackEntry, Steering
...
if TYPE_CHECKING:
    ...
    from drowse.core.instruments.protocol import Instrument
    from drowse.core.instruments.sae import SaeInstrument
    from drowse.core.scoring import ChoiceScores
...
_JLENS_DEVICE_STACK_CACHE_MAX = 2
_JLENS_DECODE_CACHE_MAX = 4096
_JLENS_SELECTOR_CACHE_MAX = 64
...
def _install_la_cache_patch() -> bool:
    ...
    if getattr(LinearAttentionLayer, "_drowse_crop_patched", False):
        return False
    ...
    linear_attention_layer._drowse_save_snapshot = _save_la_snapshot
    linear_attention_layer._drowse_crop_patched = True
    return True
...
def _snapshot_la_layers(cache: Any) -> None:
    ...
    Called from :meth:`DrowseSession.cache_prefix` right after the
    ...
    if not layers:
        return
    for layer in layers:
        save = getattr(layer, "_drowse_save_snapshot", None)
        if save is not None:
            # ``_drowse_save_snapshot`` is ``setattr``-ed on the
...
# An EOS-only sampled row carries no conversational evidence and cannot be
# installed as a manifold node.  Retry only that aligned row greedily while
# suppressing EOS long enough to require a short content continuation.
_CORPUS_RETRY_MIN_TOKENS = 8
...
class _SessionStopCriteria(StoppingCriteria):
    """Bridge ``DrowseSession.stop()`` into ``transformers.generate``."""
...
def _role_for(slug: str, kind: str | None) -> str | None:
    ...
    at render (:func:`drowse.core.role_templates._render_label`).  ``custom``
...
def _split_composite_source(
    ...
    interface so :meth:`DrowseSession.generate_responses` elicits
...
class GenState(IntEnum):
    ...
    Transitions live in :meth:`DrowseSession._generation_transaction` and
    :meth:`DrowseSession._generate_core`:
...
class PreparedCall:
    ...
    Produced once by :meth:`DrowseSession._prepare_generation_call` and read by
...
class ReadDemand:
    ...
    :meth:`DrowseSession._resolve_read_demand` and consumed by
    :meth:`DrowseSession._begin_capture`, which chooses the physical capture
...
class _GenerationTransaction:
    """The mutable state :meth:`DrowseSession._generation_transaction` owns.
...
class ConcurrentGenerationError(RuntimeError, DrowseError):
...
class ProfileNotRegisteredError(KeyError, DrowseError):
...
class ManifoldNotRegisteredError(KeyError, DrowseError):
...
class ConcurrentExtractionError(RuntimeError, DrowseError):
    ...
    top of :meth:`DrowseSession.extract` (the pipeline itself is unaware
    of generation state).
    """
...
def _affine_manifold_push(
    ...
    if isinstance(position, str):
        ...
        if len(pos) != n:
            from drowse.core.errors import ManifoldArityError
...
class _SteeringContext:
    """Context manager returned by DrowseSession.steering().
    ...
    an :class:`~drowse.core.steering_expr.AblationTerm` for mean-replacement
...
class DrowseSession:
    ...
    def from_pretrained(
        ...
        *,
        ...
        if cuda_graphs:
            from drowse.core.static_cache import is_cuda_graphs_supported
        ...
        if compile and device_obj.type in ("cuda", "mps"):
            ...
            from drowse.core.model import _compile_with_probe, get_layers
    ...
    def __init__(
        ...
        *,
        ...
        # is logged once via :func:`drowse.core.static_cache.warn_once`
        ...
        if cuda_graphs_requested:
            from drowse.core.static_cache import (
        ...
        # Drowse-trained weights live under DROWSE_HOME. Readout probes are
        ...
        from drowse.io.sae import load_active_sae
    ...
    def model(self) -> torch.nn.Module:
        """Live HF model.  Part of the :class:`~drowse.core.extraction.ModelHandle` protocol."""
        return self._model

    @property
    def tokenizer(self):
        """Live HF tokenizer.  Part of the :class:`~drowse.core.extraction.ModelHandle` protocol."""
        return self._tokenizer

    @property
    def device(self) -> torch.device:
        """Model device.  Part of the :class:`~drowse.core.extraction.ModelHandle` protocol."""
        return self._device

    @property
    def dtype(self) -> torch.dtype:
        """Model dtype.  Part of the :class:`~drowse.core.extraction.ModelHandle` protocol."""
        return self._dtype

    @property
    def layers(self):
        """Layer-list accessor.  Part of the :class:`~drowse.core.extraction.ModelHandle` protocol.
    ...
    def analytics_names(self) -> list[str]:
        ...
        from drowse.core.capture import is_foldable_vector_manifold
    ...
    def joint_logprob_cache(self) -> dict[tuple[str, str], Any]:
        ...
        API keeps this cache bounded and invalidates it after tree edits."""
    ...
    def layer_means(self) -> dict[int, torch.Tensor]:
        ...
        if not self._layer_means:
            with self._model_exclusive(
                ...
                DrowseSession._assert_unsteered_artifact_operation(self)
                ...
                if not self._layer_means:
                    ...
                    if not means:
                        raise DrowseError("neutral layer-mean bootstrap returned empty")
    ...
    def whitener(self) -> "Any":
        ...
        :class:`drowse.core.mahalanobis.LayerWhitener` or ``None`` if
    ...
    def jlens(self) -> "Any":
        ...
        immutable Drowse-owned shards; external bindings read provider-owned
        cache bytes. Fit or fetch one with ``drowse lens fit|fetch``. Returns a
        :class:`drowse.core.jlens.JacobianLens`.
        ...
        with self._lens_instrument.state_lock:
            ...
            if sidecar is None:
                if self._jlens is not None:
                    DrowseSession._evict_resident_jlens(self)
                return None
            ...
            if not _jlens_matches_loaded_model(
                ...
                return None
            DrowseSession._adopt_fitted_jlens(
    ...
    def _require_jlens(self) -> "Any":
        ...
        if lens is None:
            raise LensNotFittedError(
                ...
                f"`drowse lens fetch {self.model_id}` for a supported official "
                f"lens, or `drowse lens fit {self.model_id}`"
            )
        return lens
    ...
    def select_jlens_source(self, source: str) -> None:
        ...
        """
        from drowse.io.lens_sources import use_lens_source
    ...
    def fit_jlens(
        ...
        *,
        ...
        """
        from drowse.io.lens import cleanup_lens_artifacts, lens_fit_lock
        ...
        with lens_fit_lock(self.model_id, lens_name):
            ...
            return DrowseSession._fit_jlens_transaction(
                self,
                prompts,
    ...
    def _fit_jlens_transaction(
        ...
        *,
        ...
        through the model (the only backward passes in drowse).
        """
        import hashlib
        ...
        with self._model_exclusive(
            ...
            def _run_estimator(*args: "Any", **kwargs: "Any") -> "Any":
                if backward_rules == "relp":
                    from drowse.core.relp import relp_backward_rules
            ...
            model_source_fp = getattr(
                fit_model, "_drowse_source_fingerprint", None,
            )
            ...
            if base is not None and not usable:
                try:
                    ...
                    with self._lens_instrument.state_lock:
                        ...
                        return DrowseSession._adopt_fitted_jlens(
                            self,
                            merged,
                            sidecar=load_lens_sidecar(self.model_id, lens_name),
                        )
    ...
    def _jlens_transport_stack(
        ...
        while len(cache) >= _JLENS_DEVICE_STACK_CACHE_MAX:
    ...
    def _jlens_decode_id(self, token_id: int) -> str:
        ...
        tok = decode_cache.pop(token_id, None)
        if tok is not None:
            decode_cache[token_id] = tok
            return tok
        tok = str(self._tokenizer.decode([token_id]))
        while len(decode_cache) >= _JLENS_DECODE_CACHE_MAX:
    ...
    def _jlens_depths(self, layers: "Sequence[int]") -> list[float]:
        ...
        if cached is not None:
            cache.pop(layer_tuple)
            cache[layer_tuple] = cached
            return cached
        denom = max(len(self._layers) - 1, 1)
        depths = [layer / denom for layer in layer_tuple]
        while len(cache) >= _JLENS_SELECTOR_CACHE_MAX:
            cache.pop(next(iter(cache)))
    ...
    def _jlens_logits_rows(
        ...
        J_rows = DrowseSession._select_tensor_rows(
            J_unique,
    ...
    def _jlens_aggregate_rows(
        ...
        *,
        ...
        :func:`drowse.core.jlens.aggregate_readout` for the statistics)."""
        from drowse.core.jlens import (
            aggregate_readout,
    ...
    def _jlens_topk_rows(
        ...
        *,
        ...
        """
        from drowse.core.jlens import topk_probabilities
    ...
    def jlens_readout(
        ...
        *,
        ...
        :func:`drowse.core.jlens.aggregate_readout`) and returns the pair
    ...
    def jlens_token_readout(
        ...
        *,
        ...
        center of mass; :func:`drowse.core.jlens.aggregate_readout`).
        Raises :class:`~drowse.core.jlens.LensNotFittedError` with no
    ...
    def instruments(self) -> "dict[InstrumentFamily, Instrument]":
        ...
        :class:`~drowse.core.instruments.protocol.Instrument`
        implementation.
        """
        return {
    ...
    def train_sae(
        ...
        *,
        ...
        """Train and persist a Drowse-owned residual-post SAE."""
        import hashlib

        from drowse.core.sae_training import train_residual_sae
        from drowse.io.sae_artifacts import (
            local_sae_release,
    ...
    def load_sae(self, release: str, *, layer: int | None = None) -> dict[str, Any]:
        ...
        Drowse storage. The deterministic default is the covered layer nearest
        65% model depth, preferring the workspace band.
        """
        from drowse.core.sae import (
            LocalSaeBackend,
            load_sae_backend,
        ...
        from drowse.io.sae_artifacts import normalize_local_sae_name
    ...
    def _require_sae(self) -> tuple[Any, int, int]:
        from drowse.core.errors import SaeNotLoadedError
        ...
        if backend is None or layer is None or width is None:
            raise SaeNotLoadedError(
                ...
                "`drowse sae use MODEL SOURCE` "
                "or POST /drowse/v1/sessions/default/sae/load"
            )
        return backend, int(layer), int(width)

    def validate_sae_feature(self, feature_id: int | str) -> dict[str, Any]:
        from drowse.core.errors import SaeFeatureError
    ...
    def _run_generator_batch(
        ...
        *,
        ...
        if deterministic:
            ...
            generation_kwargs.update(temperature=1.0, top_p=0.9)
    ...
    def _run_corpus_batch(
        ...
        *,
        ...
        """Generate one aligned corpus chunk, repairing EOS-only rows.

        Sampling can legitimately choose EOS as its first token for one row in
        an otherwise healthy batch.  Dropping that row would shift every later
        response away from its baseline prompt, while keeping it would create an
        invalid blank node corpus.  Retry exactly the blank row with the same
        system prompt, user prompt, and role, using deterministic greedy decode
        and a short EOS suppression window.  The retry therefore remains model
        output under the identical elicitation frame and cannot perturb corpus
        order or the sampling RNG used by the other rows.
        """
        texts = self._run_generator_batch(
            system_msg, prompts, max_new_tokens, role=role,
        )
        if len(texts) != len(prompts):
            raise RuntimeError(
                "corpus generation returned a different number of responses "
                f"than prompts ({len(texts)} != {len(prompts)})"
            )
        repaired = list(texts)
        for index, text in enumerate(texts):
            if text.strip():
                repaired[index] = text.strip()
                continue
            retry = self._run_generator_batch(
                system_msg,
                [prompts[index]],
                max_new_tokens,
                role=role,
                deterministic=True,
                min_new_tokens=_CORPUS_RETRY_MIN_TOKENS,
            )
            if len(retry) != 1 or not retry[0].strip():
                raise RuntimeError(
                    "the model produced no content for corpus prompt "
                    f"{prompts[index]!r}, including the deterministic retry"
                )
            repaired[index] = retry[0].strip()
        return repaired
    ...
    def generate_responses(
        ...
        *,
        ...
        (:func:`_system_for`, led by the shared :data:`~drowse.core.capture._LENGTH_DIRECTIVE`
        ...
        :func:`~drowse.core.manifold.compute_manifold_node_stats` and the node
        corpus files assume.
        ...
        """
        from drowse.core.capture import _LENGTH_DIRECTIVE, _load_baseline_prompts
    ...
    def generate_neutral_responses(
        ...
        *,
        ...
        :data:`~drowse.core.capture._LENGTH_DIRECTIVE` as its *only* system prompt
        ...
        *,
        ...
        with _locked_manifest(folder):
            ...
            if force or not manifest.exists():
                ...
                if folder.exists():
                    from drowse.io.manifold_folder import reset_manifold_folder
    ...
    def bake(
        ...
        *,
        ...
        :func:`drowse.io.bake.merge_into_manifold`, model-scoped to this
        ...
        """
        from drowse.io.bake import MergeError, merge_into_manifold
    ...
    def steering(
        ...
        the shared grammar in :mod:`drowse.core.steering_expr`) or a
    ...
    def _bootstrap_manifold_probes(
        ...
        - **Tagged concept axes** — for each bundled 2-node ``pca`` manifold
          ...
          :func:`~drowse.io.probes_bootstrap.load_default_manifolds`) fit-or-
        ...
        """
        from drowse.io.probes_bootstrap import load_default_manifolds
    ...
    def _steering_needs_probe_gating(self) -> bool:
        ...
        :class:`~drowse.core.triggers.ProbeGate`.
    ...
    def score_choices(
        ...
        *,
        ...
        :func:`drowse.core.scoring.score_choices`.
        """
        from drowse.core.scoring import score_choices as _score_choices
    ...
    def score_template(
        ...
        *,
        ...
        ``template`` is a :class:`~drowse.io.templates.TemplateFolder` or a
        selector string (``<name>`` / ``<ns>/<name>``). Returns one
        :class:`~drowse.core.scoring.ChoiceScores` per context.
        """
        from drowse.core.scoring import score_template as _score_template
        from drowse.io.templates import TemplateFolder, resolve_template
    ...
    def _aggregate_forward_index(self, generated_ids: list[int]) -> int | None:
        ...
        if (
            ...
            return None
        from drowse.core.capture import last_content_index
    ...
    def score_hidden(
        ...
        *,
        ...
        layers raise :class:`DrowseError`. Empty dict raises.
        ...
        """
        if not hidden:
            raise DrowseError("score_hidden: no layers provided")
        ...
        # violating the "all public errors are DrowseError" invariant.
        ...
        # catching DrowseError get a uniform exception surface.
        try:
            agg, per_tok = self._monitor.score_stack(
                hidden, accumulate=accumulate,
            )
        except ValueError as exc:
            raise DrowseError(f"score_hidden: {exc}") from exc
    ...
    def _probe_hash(self, name: str) -> str | None:
        ...
        Stamps :class:`drowse.core.loom.Recipe.probe_hashes` so transcript
    ...
    def diff_nodes(self, a_id: str, b_id: str) -> Any:
        """Return a :class:`drowse.core.loom_diff.NodeDiff` between two nodes.
        ...
        otherwise.
        """
        from drowse.core.loom_diff import NodeDiff, readings_diff, text_diff
    ...
    def regen_with_modifier(
        ...
        *,
        ...
        ``"inverted"``, ``"reseed"``, ``"cool"``, ``"hot"``), a
        comma-separated partial-recipe string, or a partial :class:`Recipe`.
        :meth:`Recipe.compose_modifier` parses strings and passes Recipe
        instances through unchanged. The override
        ...
        # compose_modifier handles built-ins, custom strings, and Recipe
        # instances — the dispatch lives on the dataclass.
    ...
    def fork_from_token(
        ...
        *,
        ...
        """Regenerate ``node_id`` as a sibling with a replacement at one token.

        Replays the assistant node's raw decode sequence up to
        ``raw_index``, forces either ``alt_token_id`` or the tokenization of
        ``replacement_text`` at that position, then samples the continuation
        freely.  Authored text may span multiple model tokens.  The node's
        steering, sampling, and thinking recipe is reused.  By default the
        original seed is re-applied and the sampler still draws every forced
        step, preserving the RNG stream through the fork point for a clean
        counterfactual.  Passing ``seed`` instead requests a newly sampled
        ending while keeping the forced prefix fixed.
        ...
        if (alt_token_id is None) == (replacement_text is None):
            raise InvalidNodeOperationError(
                "fork_from_token: provide exactly one alternative token id "
                "or replacement text"
            )
        if replacement_text is not None:
            replacement_ids = [
                int(t) for t in self._tokenizer.encode(
                    replacement_text, add_special_tokens=False,
                )
            ]
            if not replacement_ids:
                raise InvalidNodeOperationError(
                    "fork_from_token: replacement text tokenized to an empty "
                    "sequence"
                )
        else:
            assert alt_token_id is not None
            replacement_ids = [int(alt_token_id)]
        forced_prefix = [int(t) for t in raw[:raw_index]] + replacement_ids
    ...
    def _loom_conflict_check(self, node_id: str, op: str) -> None:
        """Tree-mutation conflict checker — see :mod:`drowse.core.loom`.
    ...
    def cache_prefix(
        ...
        *,
        ...
        if use_static:
            try:
                from drowse.core.static_cache import make_static_cache
    ...
    def _finalize_generation(
        ...
        *,
        ...
        from drowse.core.generation_finalizer import finalize_generation
    ...
    def _resolve_recipe_override(
        ...
        *,
        ...
        ``recipe_override`` is a :class:`Recipe` partial (None fields fall
        through), a built-in mode string, or a comma-separated partial-recipe
        string forwarded to :meth:`Recipe.compose_modifier`. The override composes onto the
    ...
    def _generate_core(
        ...
        *,
        ...
        with self._generation_transaction() as txn:
            ...
            from drowse.core.token_payloads import (
                TokenProbePayload,
        ...
        *,
        ...
        Args:
            ...
            n: fan-out count.  ``n=1`` (default) returns a one-result
                ...
                seeds (see :func:`~drowse.core.loom.derive_seed_schedule`)
    ...
    def _generate_batch_fast(
        ...
        *,
        ...
        ``None`` means "fall back to the serial Drowse decode loop."  The fast
        ...
        streams stay on the serial Drowse loop.
        """
    ...
    def _batch_generated_tokens(
        ...
        *,
        ...
        """Trim HF batch output into Drowse' generated-token convention."""
    ...
    def generate_batch(
        ...
        *,
        ...
        Anything that needs Drowse' row-shaped custom loop (stateful history,
    ...
    def close(self) -> None:
        ...
        persistent capture buffers — and ``DrowseSession.__init__`` accepts a
        ...
        for instrument_name in (
            "_geometry_instrument", "_lens_instrument", "_sae_instrument",
        ):
            instrument = self.__dict__.get(instrument_name)
            if instrument is None:
                continue
            instrument.close_run()
            if hasattr(instrument, "probes"):
                instrument.probes.clear()
            if hasattr(instrument, "live"):
                instrument.live = None

        monitor = getattr(self, "_monitor", None)
        if monitor is not None:
            for name in list(monitor.probe_names):
                monitor.remove_probe(name)
            monitor.set_whitener(None)

        self._profiles.clear()
        self._manifolds.clear()
        for cache_name in (
            "_analytics_cpu_cache",
            "_joint_logprob_cache",
            "_probe_hash_cache",
            "_jlens_device_cache",
            "_jlens_depths_cache",
            "_jlens_depth_tensor_cache",
            "_readout_long_tensor_cache",
            "_jlens_decode_cache",
            "_sae_feature_meta",
            "_layer_means",
    ...
    def __enter__(self) -> "DrowseSession":
        return self
