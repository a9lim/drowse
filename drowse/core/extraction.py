...
:class:`EventBus` for ``ManifoldExtracted`` emission.  ``DrowseSession``
...
from drowse.core.topology import PcaDiagnostics, SpectralDiagnostics
from drowse.core.model import loaded_model_fingerprint, workspace_layer_indices
from drowse.core.sae import SaeBackend
from drowse.io.paths import model_dir, tensor_filename
...
def _apply_fit_overrides_locked(
    ...
    *,
    ...
    from drowse.io.manifolds import ManifoldFolder, sanitize_hyperparams
...
class ManifoldAuthoringChangedError(RuntimeError, DrowseError):
    """The authoring inputs changed while a target was being fitted.

    Semantically the conflict :class:`~drowse.core.session.ConcurrentExtractionError`
...
def _gc_capture_generations(
    ...
    from drowse.io.atomic import fsync_directory
...
def _commit_capture_generation(
    ...
    *,
    ...
    """
    from drowse.io.atomic import fsync_directory, write_json_atomic
...
def _capture_generation_is_complete(
    ...
    for idx in centroid_layers:
        ...
        if (
            ...
            return False
    from drowse.io.integrity import verify_integrity
...
def _prune_manifold_capture_cache(
    ...
    """
    try:
        limit_gb = float(os.environ.get("DROWSE_MANIFOLD_CAPTURE_CACHE_GB", "8"))
    except ValueError:
        limit_gb = 8.0
    if limit_gb <= 0.0:
        return
    limit = int(limit_gb * 1024**3)
    from drowse.io.atomic import artifact_has_live_lease, artifact_lock
...
# implementation.  ``DrowseSession`` satisfies each by virtue of carrying
...
class ModelHandle(Protocol):
    ...
    def _run_generator(
        ...
        protocol shape mirrors the existing ``DrowseSession._run_generator``
...
def prepare_capture_rows(
    ...
    *,
    ...
    """
    from drowse.core.capture import _prepare_capture_batch
...
def _authoring_snapshot_locked(
    ...
    template_sha: str | None = None
    if mf.template_ref is not None:
        from drowse.io.manifold_authoring import _write_node_corpus
        from drowse.io.templates import resolve_template
...
def _resolve_fit_geometry(
    ...
    *,
    ...
    from drowse.core.topology import discover_coords, select_topology
    from drowse.io.manifolds import min_nodes, sanitize_hyperparams
...
class ManifoldExtractionPipeline:
    ...
    :class:`~drowse.core.manifold.LayerSubspace` is model-space, so the
    ...
    def fit(
        ...
        *,
        ...
        emission: :meth:`DrowseSession._fit_concept_manifold` re-emits a single
        ...
        """
        from drowse.io.atomic import artifact_lock
        from drowse.io.manifold_folder import _locked_manifest, manifold_pair_lock
        ...
        fit_lock = folder_path.parent / f".drowse-fit-{fit_lock_key}"
    ...
    def _fit_locked(
        ...
        *,
        ...
        :func:`drowse.core.topology.discover_coords`).  Returns a
        :class:`~drowse.core.manifold.Manifold`.
        ...
        from drowse.core.topology import neutral_layout_coord
        from drowse.core.capture import compute_dls_axes
        from drowse.io.manifolds import ManifoldFolder, ManifoldSidecar
        ...
        model_source_fp = getattr(
            base_model, "_drowse_source_fingerprint", None,
        )
        ...
        if not force and not _verified_cache_miss:
            from drowse.io.manifold_tensors import _load_manifold_locked
        ...
        if capture_layers:
            ...
            from drowse.core.capture import _ReusablePooledCapture
