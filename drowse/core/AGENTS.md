...
table — both exist for the Jacobian-lens readout, and nothing else in drowse
...
`DrowseSession._begin_capture` sets it and every `_score_*` dispatch keys off
...
apply). `DrowseSession.whitener` is a lazy property wired into the `Monitor` via
`set_whitener`.
...
Current validation boundary (September 2026): this is a bounded periodic-chart
proposer, not an arbitrary-manifold classifier. H1 evidence includes eligible
finite bars; checked distance-MDS quadratic forms untangle circle factors before
a guarded spectral fallback. Ambient rank, coverage, and neighborhood-collapse
checks reject unsupported charts. A periodic RBF must beat mean-only GCV before
it can take priority over a flat approximation. Triangle-budget exhaustion
raises rather than truncating. Sphere/Mobius type inference and some thin/3D
tori remain unresolved; see `browser-runtime/TOPOLOGY_VALIDATION_2026-09-05.md`.
The historical fixture-specific loop-recovery examples below are not universal
guarantees. Existing automatic artifacts require explicit forced refitting.
...
`ManifoldExtracted`; `DrowseSession` satisfies it implicitly.
...
oldest-first past 8 GiB (`DROWSE_MANIFOLD_CAPTURE_CACHE_GB`); `pack clear` /
...
provider releases; `LocalSaeBackend` adapts Drowse-trained artifacts.
...
code in drowse**. Everything else runs under `inference_mode`, and inference
...
back to exact unreplicated scalar VJPs, and `DROWSE_JLENS_VJP=replicated` keeps the
...
collaborator, instantiated once in `DrowseSession.__init__` after the Monitor. It
...
base-model branch. `DrowseSession.scene_grammar` is the lazy per-session
...
`DrowseSession._token_tap` absorbs before invoking user callbacks.
...
`DrowseSession` owns the model, the in-memory profile registry (`_profiles`), the
...
`DROWSE_TRANSCRIPT_VERSION = 2` (per-turn `speaker:` = `role_label`, `thinking:` =
...
typed field feeding the OpenAI/Ollama `x-drowse-probe-readings` vendor extension
...
`{format_version, drowse_version, method, tensor_sha256, provenance}`; readers
...
`errors.py` — `DrowseError` is the base; every drowse exception multi-inherits
through it while keeping its stdlib MRO, so `except DrowseError` catches the
...
modules re-export the class they raise (`from drowse.core.mahalanobis import
...
`$DROWSE_HOME`); and no dict input to `Steering`.
