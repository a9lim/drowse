...
This class is purely the Python-level surface the rest of drowse uses so
...
# ``ProfileError`` is defined in :mod:`drowse.core.errors` (the single home
# for the engine's error taxonomy) and re-exported here so
# ``from drowse.core.profile import ProfileError`` keeps working.
from drowse.core.errors import ProfileError as ProfileError
...
_PROFILE_SIDECAR_FIELDS = {
    "format_version", "drowse_version", "method", "tensor_sha256",
    "provenance",
}
...
_PROFILE_STAMPED_FIELDS = frozenset({
    "format_version", "drowse_version", "tensor_sha256",
})

# The live producers of a profile sidecar: a bare fold / hand-built
# ``Profile.save`` (``profile``), ``DrowseSession.extract``'s folded 2-node
...
def _validate_profile_sidecar(
    ...
    if isinstance(data, dict) and (
        ...
        raise ProfileError(
            ...
            f"{PROFILE_FORMAT_VERSION}. Re-save it with the current drowse "
            "(for an extracted concept, re-run `drowse manifold extract`)."
        )
    ...
    if (
        not isinstance(data["drowse_version"], str)
        or not data["drowse_version"]
    ):
...
def save_profile(
    ...
    :meth:`DrowseSession.extract` returns) or ``"merge"``
    (:meth:`Profile.merged`).
    ...
    (``format_version`` / ``drowse_version`` / ``tensor_sha256``) are
    ...
    # fp32 write invariant: every drowse safetensor writer enforces fp32
    ...
    from drowse.io.atomic import artifact_lock, write_bytes_atomic, write_json_atomic
...
def load_profile(path: str | pathlib.Path) -> tuple[dict[int, torch.Tensor], dict[str, Any]]:
    ...
    in the GGUF header (see :mod:`drowse.io.gguf_io`). Both paths yield the
    ...
    path = pathlib.Path(path)
    if path.suffix == ".gguf":
        from drowse.io.gguf_io import read_gguf_profile
    ...
    metadata = migrate_legacy_record(metadata)
    tensor_digest = hashlib.sha256(tensor_bytes).hexdigest()
    ...
    out_metadata: dict[str, Any] = {
        key: metadata[key] for key in ("format_version", "drowse_version", "method",
                                       "tensor_sha256")
    }
...
class Profile:
    ...
    def to_gguf(self, path: str | pathlib.Path, *, model_hint: str) -> None:
        ...
        uniform ``--control-vector-scaled`` scalar reproduces drowse's
        per-layer weighting without needing a per-layer metadata slot.
        """
        from drowse.io.gguf_io import write_gguf_profile
