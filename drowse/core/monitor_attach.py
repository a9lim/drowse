"""Attach-time probe algebra for :class:`~drowse.core.monitor.Monitor`.
...
attribute resolution overhead.  :mod:`drowse.core.monitor` imports it (and the
supporting constants / dataclasses) back from here; this module must **not**
import from :mod:`drowse.core.monitor` (no circular dependency).
"""
...
class AttachedManifoldProbe:
    """One manifold registered on a :class:`~drowse.core.monitor.Monitor`.

    Pairs the loaded :class:`~drowse.core.manifold.Manifold` artifact with
...
class _LayerWhiten:
    ...
    Built at attach from the wired :class:`~drowse.core.mahalanobis.LayerWhitener`
    ...
    (:meth:`~drowse.core.mahalanobis.LayerWhitener.subspace_gram`);
...
def _build_whitened_factors(
    ...
    *,
    ...
    :class:`~drowse.core.mahalanobis.WhitenerError` — there is no Euclidean
...
def _compute_assign_bandwidth(
    ...
    * **curved + σ-field**: each node's reduced within-node thickness
      ``σ(z)`` (:meth:`~drowse.core.manifold.LayerSubspace.sigma_at`) mapped
...
def _layer_geometry(
    ...
    * ``invert_query`` — ``(R,)`` M-orthogonal projection coords
      ``c = M_R⁻¹ g`` for :func:`~drowse.core.manifold.invert_parameterization`.
