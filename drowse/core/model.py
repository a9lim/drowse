...
_ORIG_HISTC = None
_ORIG_LDEXP = None
_MODEL_FILE_HASH_CACHE_MAX = 4096
...
def invalidate_loaded_model_fingerprint(model: Any) -> None:
    ...
    Drowse-owned models are immutable after load; steering is per-call and does
    ...
    setattr(base, "_drowse_state_invalidated", True)
...
def _local_model_files(path: Path) -> list[tuple[str, int, str]]:
    ...
    for relative, file_path in sorted(found.items()):
        ...
        cached = _MODEL_FILE_HASH_CACHE.pop(key, None)
        ...
        if cached is not None and cached[:-1] == identity:
            file_hash = cached[-1]
            _MODEL_FILE_HASH_CACHE[key] = cached
        else:
            file_hash = hash_file(file_path)
            while len(_MODEL_FILE_HASH_CACHE) >= _MODEL_FILE_HASH_CACHE_MAX:
                _MODEL_FILE_HASH_CACHE.pop(next(iter(_MODEL_FILE_HASH_CACHE)))
...
def model_source_fingerprint(
    ...
    try:
        ...
        raw_config_path = str(getattr(config, "_name_or_path", "")).strip()
        ...
        local_files = _local_model_files(local_path) if local_path is not None else []
...
def loaded_model_fingerprint(model: Any, model_id: str) -> str:
    ...
    trusted_source = getattr(base, "_drowse_source_fingerprint", None)
    payload = {
        "model_id": model_id,
    ...
    loaded_signature = getattr(base, "_drowse_loaded_state_signature", None)
    if (
        not trusted_source
        or state_signature != loaded_signature
        or bool(getattr(base, "_drowse_state_invalidated", False))
    ):
        # No trusted load provenance exists (programmatically constructed,
        # separately initialized, externally wrapped, or mutated after drowse
        ...
        # drowse-owned loads use the exact immutable source fingerprint above.
...
def patch_torch_for_mps() -> bool:
    ...
    functions, but drowse now installs them only when an MPS model load is
    actually requested.
    """
    global _ORIG_HISTC, _ORIG_LDEXP
    installed = False
    if getattr(torch.histc, "_drowse_mps_safe", False) is False:
        _ORIG_HISTC = torch.histc
        _histc_mps_safe._drowse_mps_safe = True  # pyright: ignore[reportFunctionMemberAccess]  # FunctionType stub forbids dynamic attrs
        torch.histc = _histc_mps_safe
        installed = True
    if getattr(torch.ldexp, "_drowse_mps_safe", False) is False:
        _ORIG_LDEXP = torch.ldexp
        _ldexp_mps_safe._drowse_mps_safe = True  # pyright: ignore[reportFunctionMemberAccess]  # FunctionType stub forbids dynamic attrs
...
class ArchProfile:
    """What drowse knows about one ``model.config.model_type``.
...
def _run_compile_probes(compiled: Any, model: PreTrainedModel, device: str | torch.device, bos_token_id: int, *, mode: str) -> None:
    """Trigger compilation of the call shapes drowse actually uses.

    For ``mode="reduce-overhead"`` drowse's session generates through
    ...
    def _probe_static(prefill_len: int) -> None:
        ...
        try:
            ...
            return
        # Prefill — drowse's generation passes attention_mask on prefill.
        ids = torch.full(
            (1, prefill_len), bos_token_id, device=device, dtype=torch.long,
        )
        attn = torch.ones_like(ids)
        ...
        del out
        # Decode — drowse passes ``attention_mask=None`` on decode steps;
    ...
    *,
    ...
    try:
        ...
        warnings.warn(
            ...
            "(DrowseSession.from_pretrained) to silence this warning.",
            UserWarning,
            stacklevel=2,
        )
...
def _resolve_load_plan(
    ...
    *,
    ...
    # text model drowse supports, load *only* the text submodel: it skips
    ...
    # multimodal model is still usable when handed to DrowseSession
...
def _materialize_model(plan: LoadPlan) -> Any:
    ...
    try:
        ...
        if (
            ...
            # error.
            raise RuntimeError(
                f"{plan.model_id!r} is a multimodal checkpoint drowse can "
...
def _finalize_model(
    ...
    *,
    ...
    # as Drowse compared it with the live text model.  The revision was already
    ...
    model.train(False)
    # Mark only drowse-owned loads as source-trusted.  Arbitrary modules whose
    ...
    if source_fingerprint is not None:
        setattr(model, "_drowse_source_fingerprint", source_fingerprint)
        setattr(model, "_drowse_loaded_state_signature", _model_state_signature(model))
        setattr(model, "_drowse_state_invalidated", False)
    ...
    from drowse.core.generation import _get_eos_ids
...
def get_layers(model: PreTrainedModel) -> nn.ModuleList:
    ...
    if model_type not in _TESTED_ARCHS and model_type not in _warned:
        ...
        warnings.warn(
            ...
            "report issues at https://github.com/a9lim/polythetic",
            UserWarning,
            stacklevel=2,
        )
