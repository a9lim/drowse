...
def _commit_pinned_converter(converter: Any, revision: str) -> Any:
    ...
    lets drowse inject an immutable revision without process-global monkeypatches.
    """
    import inspect
...
class SaeBackend(Protocol):
    ...
    layers: frozenset[int]      # drowse 0-indexed transformer-block layers
...
class LocalSaeBackend:
    """Lazy adapter for one Drowse-owned residual-post SAE artifact."""
...
def list_sae_releases(model_id: str) -> list[dict[str, Any]]:
    ...
    Metadata-only: no SAE weights are loaded and no model forward runs. Drowse
    ...
    from drowse.io.sae_artifacts import list_local_saes
    ...
    try:
        ...
        raise SaeBackendImportError(
            "SAE release discovery requires `sae_lens`; reinstall drowse"
        ) from exc
...
def _validate_residual_hook(sae: Any, release: str, layer_idx: int) -> None:
    ...
    Drowse's capture layer is the transformer block output. Feeding that tensor
    ...
    if not is_residual_post:
        raise SaeCoverageError(
            f"SAE release {release!r} targets {hook_name!r}, but drowse SAE "
...
def load_sae_backend(
    ...
    *,
    ...
    """
    from drowse.io.sae_artifacts import is_local_sae_release
    ...
    try:
        ...
        raise SaeBackendImportError(
            ...
            f"installed. Reinstall drowse to restore its core dependencies."
        ) from e
    if sae_lens is None:
    ...
    if registry_model and model_id and not _model_names_match(registry_model, model_id):
        raise SaeModelMismatchError(
            ...
            f"the drowse session is loaded with '{model_id}'"
        )
...
def _model_names_match(a: str, b: str) -> bool:
    ...
    drowse callers pass full HF ids (``openai-community/gpt2``). We match
