...
``@when:`` (in :mod:`drowse.core.steering_expr`) gates on *per-step*
...
``GET /drowse/v1/sessions/{id}/tree/filter``).
...
class FilterParseError(ValueError, DrowseError):
...
def node_token_series(node: Any, probes: frozenset[str]) -> dict[str, list[float]]:
    ...
    import-cycle-free against :mod:`drowse.core.loom`.
    """
...
class FilterClause:
    ...
    :class:`drowse.core.loom.LoomNode` via :meth:`evaluate`.  Stored as a
    ...
    def evaluate(self, node: Any) -> bool:
        ...
        stays import-cycle-free against ``drowse.core.loom``).  ``agg:``
...
def filter_tree(tree: Any, text: str) -> set[str]:
    ...
    ``tree`` is a :class:`drowse.core.loom.LoomTree`.  Returns the set
