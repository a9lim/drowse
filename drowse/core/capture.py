...
class CaptureMode(Enum):
    ...
    when it scores; :meth:`DrowseSession._begin_capture` picks it and every
...
class CaptureState:
    ...
    :meth:`DrowseSession._begin_capture`; read by the ``_score_*`` dispatch,
...
def last_content_index(
    ...
    """Index of the final token belonging to the assistant content span.

    Chat templates may append whitespace *after* their closing special token
    (Smol ChatML renders ``...content\n<|im_end|>\n``). Trim terminal
    whitespace only when it follows a closing id, then walk backward past ids from
    :func:`special_token_ids` (``all_special_ids`` + ``added_tokens_encoder``).
    Whitespace before the closing marker remains part of the assistant span;
    its state is the one that predicts the marker, matching browser capture.
    The result floors at 0 so an all-structural sequence still yields a valid
    index.  This is the single definition of "last content token" shared by
    extraction (:func:`_encode_and_capture_all`), the
    ...
    # A template-owned newline can follow the closing marker. It is not an
    # added/special token, so the old special-only walk incorrectly pooled it.
    # Decode only the short suffix we inspect; scanning the whole vocabulary to
    # precompute whitespace ids would be substantially more work and is not
    # needed on this cold capture path.
    decode = getattr(tokenizer, "decode", None)
    if callable(decode):
        suffix_start = idx
        while suffix_start > 0:
            piece = decode([int(token_ids[suffix_start])], skip_special_tokens=False)
            if not isinstance(piece, str) or not piece or not piece.isspace():
                break
            suffix_start -= 1
        if int(token_ids[suffix_start]) in skip:
            idx = suffix_start
    if skip:
...
def _render_capture_text(
    ...
    *,
    ...
    if getattr(tokenizer, "chat_template", None) is not None:
        ...
        from drowse.core.role_templates import apply_with_role
...
def _render_and_tokenize_for_capture(
    ...
    *,
    ...
    :func:`drowse.core.role_templates.apply_with_role` for the persona-baselined
...
def _load_json_with_override(user_path: Path, resource_name: str) -> list[str]:
    """Read a bundled ``drowse.data`` JSON list, or the user's override of it.
    ...
    if user_path.exists():
        with open(user_path) as f:
            return json.load(f)
    with _resources.files("drowse.data").joinpath(resource_name).open() as f:
        return json.load(f)


def _load_neutral_prompts() -> list[str]:
    """Load neutral prompts, preferring a user override at ~/.drowse/neutral_statements.json."""
    from drowse.io.paths import neutral_statements_path
...
def _load_baseline_prompts() -> list[str]:
    ...
    Prefers a user override at ``~/.drowse/baseline_prompts.json``, else the
    bundled ``drowse/data/baseline_prompts.json`` (48 affect-neutral prompts).
    Conversational capture pairs each node response with its prompt by
    ``response[i] -> prompt[i % len(prompts)]``.
    """
    from drowse.io.paths import baseline_prompts_path
    return _load_json_with_override(
...
def compute_neutral_activations(
    ...
    *,
    ...
    ``X.mean(0)`` (:func:`drowse.io.probes_bootstrap.bootstrap_layer_means`).
    Used by cross-model alignment (:func:`drowse.io.alignment.fit_alignment`)
    ...
    surface :meth:`~drowse.core.session.DrowseSession.fit` and
    ...
    :func:`drowse.io.alignment.load_or_compute_neutral_activations`.
    """
    if device is None:
    ...
    with _ReusablePooledCapture(
        ...
        while start < n:
            ...
            try:
                ...
                from drowse.core.errors import is_out_of_memory_error
...
def fold_directions_to_subspace(
...
) -> "Any":  # -> drowse.core.manifold.Manifold
    ...
    *not* :func:`~drowse.core.manifold.subspace_share`'s √2-scaled form; the
...
def project_profile(
    ...
    *,
    ...
    The whitener (a :class:`drowse.core.mahalanobis.LayerWhitener`) is
