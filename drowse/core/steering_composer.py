"""SteeringComposer — the steering-resolution + stack collaborator for DrowseSession.

Extracted from :mod:`drowse.core.session` (the one cluster whose seam is clean and
...
:class:`~drowse.core.hooks.SteeringManager`, hook (re)build, and the probe-gate
plumbing.
...
from drowse.core.events import SteeringApplied, SteeringCleared
...
if TYPE_CHECKING:
    from drowse.core.instruments.types import GateRef, InstrumentFamily
...
def _fires_in_prefill(trigger: Trigger) -> bool:
    ...
    :meth:`~drowse.core.triggers.Trigger.active`).  Both the live-stack check
...
class SteeringComposer:
    ...
    Instantiated once per :class:`~drowse.core.session.DrowseSession` after the
    ...
    def __init__(self, session: "DrowseSession") -> None:
    ...
    def materialize_projections(self, steering: "Steering") -> dict[str, object]:
        ...
        :class:`~drowse.core.steering_expr.ProjectedTerm` in
        ``steering.alphas``.

        Ensures the ``base`` and ``onto`` profiles are loaded (invoking
        the autoload path when needed), runs
        :func:`drowse.core.capture.project_profile` to build the derived
        ...
        raises :class:`~drowse.core.mahalanobis.WhitenerError` otherwise;
        ...
        """
        from drowse.core.steering_expr import ProjectedTerm
    ...
    def ensure_manifold_loaded(self, key: str) -> None:
        ...
        from drowse.io.paths import manifold_dir, manifolds_dir, tensor_filename
        ...
        from drowse.io.manifold_folder import fitted_sidecar_is_current
        ...
        if not matches:
            raise ManifoldNotRegisteredError(
                ...
                f"run `drowse manifold fit` first"
            )
        if len(matches) > 1:
        ...
        if metadata.get("model_fingerprint") != live_fingerprint:
            raise ManifoldNotRegisteredError(
                ...
                f"under {model_id}; run `drowse manifold fit` again"
            )
        if metadata.get("fit_mode", "authored") != "baked":
            from drowse.core.extraction import prepare_manifold_capture_identity
    ...
    def ensure_profile_registered(
        ...
        2. a fitted 2-node ``pca`` manifold on disk — loaded via
           ...
           and folded by :func:`~drowse.core.capture.folded_directions`,
    ...
    def steering_needs_probe_gating(self) -> bool:
        ...
        :func:`~drowse.core.steering.entry_trigger`, so every entry shape is
    ...
    def steering_active_in_prefill(self) -> bool:
        ...
        :meth:`~drowse.core.triggers.Trigger.active`).  When this is False the
    ...
    def compose_steering_entries(
        ...
        - **push** terms (vectors, poles, ``~``/``|`` projections, affine-``%``
          ...
          :func:`~drowse.core.manifold.synthesize_subspace` and registered with
    ...
    def snapshot_steering_alphas(self) -> dict[str, float]:
        ...
        For a :class:`~drowse.core.steering_expr.ManifoldTerm`, ``entry.coeff``
