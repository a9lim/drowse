...
class SaeTrainingCancelled(RuntimeError, DrowseError):
    """Raised when a cooperative local-SAE training cancellation lands.

    Mirrors :class:`~drowse.core.jlens.JacobianLensCancelled` — the same event
    on the other background-preparation family, so it reports the same 409 and
    is caught by the same ``except DrowseError``.  ``RuntimeError`` stays in
