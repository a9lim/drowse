...
:mod:`drowse.core.manifold` (pure tensor math, no session/IO coupling).
...
pure-tensor helpers that depend only on :mod:`drowse.core.manifold`
...
def fit_behavior_manifold(
    ...
    *,
    ...
    (``sqrt``) and the same :func:`~drowse.core.manifold.fit_layer_subspace`
    PCA + RBF fit is applied there.  Returns the fitted
    :class:`~drowse.core.manifold.LayerSubspace` (the behavior manifold).
    """
...
def trajectory_naturalness(
    ...
    :class:`~drowse.core.manifold.ManifoldDomain`, and ``node_coords``
...
def compute_node_behavior_centroid(
    ...
    :func:`~drowse.core.manifold.compute_manifold_node_stats` pools: each
