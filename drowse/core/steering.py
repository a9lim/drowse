"""Per-call steering configuration for DrowseSession.generate.
...
:mod:`drowse.core.steering_expr`) or a pre-built :class:`Steering`.
...
An individual entry can carry its own :class:`~drowse.core.triggers.Trigger`
...
land as :class:`~drowse.core.steering_expr.ProjectedTerm` values and are
materialized into derived profiles by
:class:`~drowse.core.session._SteeringContext` on scope entry.

Pole aliasing is NOT resolved here — that happens inside
``DrowseSession.steering()`` (the canonical resolver site, per the plan).
...
#: :class:`~drowse.core.steering_expr.ProjectedTerm` for runtime
#: projection (materialized into a derived profile by the session), an
#: :class:`~drowse.core.steering_expr.AblationTerm` for mean-replacement
#: ablation, or a :class:`~drowse.core.steering_expr.ManifoldTerm` for
...
class Steering:
    ...
    alphas: vector name -> alpha, ``(alpha, Trigger)``, or
        :class:`~drowse.core.steering_expr.ProjectedTerm`.  Pole aliases
        (bare poles of installed bipolar vectors) are resolved when the
        steering is entered via ``DrowseSession.steering()``.
    ...
    :class:`~drowse.core.mahalanobis.LayerWhitener` — there is no
    ...
    def from_value(
        ...
        :mod:`drowse.core.steering_expr`.  ``None`` passes through (the
    ...
    def classified(self) -> "SteeringEntries":
        ...
        :class:`~drowse.core.steering_expr.ProjectedTerm` values all become
        ...
        :class:`~drowse.core.steering_expr.AblationTerm` and
        :class:`~drowse.core.steering_expr.ManifoldTerm` values are kept whole,
