"""Native ``/drowse/v1/sessions`` route group."""
...
from drowse.server.session_models import (
...
def register_session_routes(app: FastAPI) -> None:
    ...
    @app.get("/drowse/v1/sessions/{session_id}")
    ...
    @app.delete("/drowse/v1/sessions/{session_id}", status_code=204)
    def delete_session(session_id: str):
        resolve_session_id(session_id)
        logging.getLogger("drowse.api").warning(
            "DELETE /drowse/v1/sessions/%s: single-session mode, no-op",
            session_id,
        )
        return Response(status_code=204)

    @app.patch("/drowse/v1/sessions/{session_id}")
    ...
    @app.post("/drowse/v1/sessions/{session_id}/steering/validate")
    async def validate_steering(
        ...
        from drowse.core.steering_expr import format_expr, parse_expr
    ...
    @app.post("/drowse/v1/sessions/{session_id}/clear", status_code=204)
    def clear_session(session_id: str):
        resolve_session_id(session_id)
        session.clear_history()
        return Response(status_code=204)

    @app.post("/drowse/v1/sessions/{session_id}/rewind", status_code=204)
