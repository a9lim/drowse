"""WebSocket request schemas and serializers for native drowse streams."""
...
from drowse.server import request_helpers
...
class WSGenerateMessage(NativeRequest):
    ...
    generating branch (:func:`drowse.server.ws_stream._normalize_submit`).
    """
    ...
    fork_replacement_text: str | None = None
    fork_seed: int | None = Field(default=None, ge=0, le=0x7fffffff)
    ...
    def _check_mode_fields(self) -> "WSGenerateMessage":
        ...
        fork_values = (
            self.fork_node_id,
            self.fork_raw_index,
            self.fork_alt_token_id,
            self.fork_replacement_text,
        )
        is_fork = any(value is not None for value in fork_values)
        if self.fork_seed is not None and not is_fork:
            raise PydanticCustomError("fork_fields", "A continuation seed requires a token fork")
        has_alt = self.fork_alt_token_id is not None
        has_text = (
            self.fork_replacement_text is not None
            and len(self.fork_replacement_text) > 0
        )
        if is_fork and (
            self.fork_node_id is None
            or self.fork_raw_index is None
            or has_alt == has_text
        ):
...
def build_sampling_config(body: WSSamplingParams | None) -> SamplingConfig | None:
    ...
    (:func:`drowse.server.request_helpers.build_sampling_config`); this is just
