"""Native template route group — ``/drowse/v1/templates/*``.

The standalone templated-completion artifact (``drowse.io.templates``): a slot, a
...
def register_template_routes(app: FastAPI) -> None:
    """Mount the ``/drowse/v1/templates/*`` tree onto ``app``."""

    session = app.state.session

    @app.get("/drowse/v1/templates")
    ...
    @app.get("/drowse/v1/templates/{namespace}/{name}")
    ...
    @app.post("/drowse/v1/templates", status_code=201)
    ...
    @app.delete("/drowse/v1/templates/{namespace}/{name}", status_code=200)
    def delete_template(
