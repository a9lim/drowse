"""Loom-tree route schemas and serializers for native drowse routes."""
...
def cast_json(session: DrowseSession) -> dict[str, CastMemberJSON]:
...
def tree_to_json(session: DrowseSession) -> LoomTreeJSON:
...
def active_path_json(session: DrowseSession) -> ActivePathJSON:
    tree = session.tree
...
def node_json(session: DrowseSession, node_id: str) -> LoomNodeDetailJSON:
