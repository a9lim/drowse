"""Ollama-compatible API routes backed by a DrowseSession.
...
talk to drowse as a drop-in replacement. Steering passes through a non-standard
...
- A drowse server hosts exactly one model. `/api/tags` advertises it under its
...
- `/api/pull`, `/api/push`, `/api/create`, `/api/copy`, `/api/delete` are
  stubbed — drowse doesn't manage models the way Ollama does. Pull is a no-op
...
from drowse.core.session import ConcurrentGenerationError, DrowseSession
from drowse.core.steering import Steering
from drowse.server.model_names import aliases_for_session, known_model_names

log = logging.getLogger(__name__)


class OllamaBadRequest(ValueError, DrowseError):
...
def _estimate_bytes(session: DrowseSession) -> int:
...
def _param_size_label(session: DrowseSession) -> str:
...
def _quant_label(session: DrowseSession) -> str:
...
def _model_details(session: DrowseSession) -> dict[str, Any]:
    info = session.model_info
...
def _tag_entries(session: DrowseSession) -> list[dict[str, Any]]:
...
def _resolve_options(
    ...
    Non-standard drowse fields (accepted at the top level or inside options):
...
def _duration_stats(result: GenerationResult, elapsed_ns: int) -> dict[str, int]:
    ...
    All durations are in nanoseconds.  Drowse tracks tokens/sec so we split the
...
def register_ollama_routes(app: FastAPI) -> None:
    ...
    def api_version():
        ...
        return {"version": f"drowse-{__version__}"}
    ...
    async def api_show(request: Request):
        ...
        return {
            ...
            "modelfile": f"# drowse: {session.model_id}\nFROM {session.model_id}\n",
            "parameters": "",
            "template": tpl,
            "details": details,
            "model_info": {
                ...
                "drowse.loaded_model": session.model_id,
                "drowse.requested_name": name,
            },
            "capabilities": ["completion", "chat"],
        }

    # -----------------------------------------------------------------------
    # Stubs for model-management endpoints drowse doesn't implement
    # -----------------------------------------------------------------------

    @app.post("/api/pull")
    async def api_pull(request: Request):
        # No-op success if the client is asking for the loaded model (by HF id
        # or by any recognized alias); 404 otherwise.  Drowse loads models at
        ...
        if name and name.lower() not in known_model_names(session):
            ...
            return JSONResponse(status_code=404, content={
                "error": (
                    f"model '{name}' not found. drowse currently hosts: {hosted}. "
                    f"To serve a different model, restart with: drowse serve <model>"
                ),
            })
    ...
    async def api_push():
        return JSONResponse(status_code=501, content={
            "error": "drowse does not implement /api/push. Use `drowse pack push` to publish a manifold.",
        })

    @app.post("/api/create")
    async def api_create():
        return JSONResponse(status_code=501, content={
            "error": "drowse does not implement /api/create.",
        })

    @app.post("/api/copy")
    async def api_copy():
        return JSONResponse(status_code=501, content={
            "error": "drowse does not implement /api/copy.",
        })

    @app.delete("/api/delete")
    async def api_delete():
        return JSONResponse(status_code=501, content={
            "error": "drowse does not implement /api/delete.",
        })

    @app.post("/api/embeddings")
    async def api_embeddings():
        return JSONResponse(status_code=501, content={
            "error": "drowse does not implement /api/embeddings. Use the model's native embedding API.",
        })

    @app.post("/api/embed")
    async def api_embed():
        return JSONResponse(status_code=501, content={
            "error": "drowse does not implement /api/embed.",
        })
    ...
    def _check_model_or_404(body: dict[str, Any]) -> None:
        ...
        if name and name.lower() not in known_model_names(session):
            ...
            raise HTTPException(
                status_code=404,
                detail=(
                    f"model '{name}' not available. drowse hosts: {hosted}. "
                    f"Unset DROWSE_STRICT_MODEL to accept any model name."
                ),
            )
    ...
    async def _run_and_build_chat_response(
        ...
        ``DrowseError`` from ``parse_expr`` (bad/colliding ``steer``
        ...
        # Drowse-specific extension: per-attached-manifold-probe aggregate
        ...
        # ("x-drowse-probe-readings"), at the top level here because
        ...
        if is_chat:
            ...
            if mf_agg:
                payload["x-drowse-probe-readings"] = mf_agg
            return payload
        # Note: Ollama's /api/generate returns a `context` field of tokenized
        # state for stateless continuation.  Drowse doesn't round-trip that,
        ...
        *before* ``StreamingResponse`` flushes headers. Materialization
        ...
        async with acquire_session_lock(session) as acquired:
            ...
            try:
                ...
                for event in stream_iter:
                    ...
                    if mf_token is not None:
                        chunk["x-drowse-probe-readings"] = mf_token
            ...
            if is_chat:
                ...
                # omitted because drowse can't round-trip it honestly.
                final = {
                    "model": model_name,
                    "created_at": created_at,
                    "response": "",
            ...
            if mf_agg:
                final["x-drowse-probe-readings"] = mf_agg
    ...
    async def api_chat(request: Request):
        ...
        # ``DrowseError`` from ``parse_expr`` is caught by FastAPI's
