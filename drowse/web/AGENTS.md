...
Static Svelte 5 + Vite dashboard mounted at `/` by `drowse serve`. The CLI mounts
...
Svelte source lives at the repo's `webui/` (peer of `drowse/`); `cd webui && npm
run build` emits straight into `drowse/web/dist/` — no intermediate copy. CI runs
`npm ci`, `npm run check`, `npm run build`, then `git diff --exit-code
drowse/web/dist/`, so a source change that isn't accompanied by a rebuilt bundle
...
both archives still fails. Hosted WebGPU assets stay in the separate hosted build.

## Mount (`routes.py`)

`register_web_routes(app)` mounts `/assets/*`, `/icons/*`, and `/social/*` on `StaticFiles` with its default
ETag / Last-Modified revalidation — Vite emits fixed filenames
(`assets/drowse.js`, `assets/index.css`), not content-hashed ones, so an
...
and anything under `api`/`v1`/`drowse`, serves the allowlisted top-level dist
files (every top-level file except `index.html` — the favicon, theme bootstrap,
and font licences), and otherwise falls back to `index.html` for SPA routing. `full_path` is
...
`/v1/*`, `/api/*`, `/drowse/v1/*`. Repeat calls append a second mount and
...
The dashboard is the sole client of the native `/drowse/v1/*` API;
...
- `generate` — the specialist door: a bare continuation (`input: null`, used by
  ...
  (`{role, content, label?}` — the unsteered A/B shadow), the **token fork**
  (`fork_node_id` / `fork_raw_index` plus exactly one of `fork_alt_token_id`
  or `fork_replacement_text`; the latter is tokenized locally and may span
  multiple tokens), and
...
webui/src/
  ...
  lib/
    ...
    stores/                   # one slice per concern; slices import each other directly
      ...
      instruments.svelte.ts   # lens / SAE / geometry live state, sources, pack fetches
      instrumentAuthoring.svelte.ts # Python-only SAE train / J-lens fit stores
    ...
    concepts.ts               # concept-catalog helpers (category / poles)
    commands.ts               # All tools index: paletteCommands() over the drawer registry
    ...
    style/{fonts.css,tokens.css,global.css}
                              # fonts.css: self-hosted Wix Madefor interface/reading
                              # families and Martian Mono for data
  panels/
    ...
    {JLens,Sae}SourceSection.svelte # Python source rows with local fit/train
    CommandPalette.svelte     # Menu → All tools (state in lib/stores/palette)
    ...
    loom/{LoomSidebar,LoomNode}.svelte  # the permanent "threads" column
...
catalog plus HF `drowse-manifold` search (debounced 300ms, server-capped at 20
rows), installing through `apiManifoldInstallStream` and rendering the SSE stage
line under the row. Both launch from the All tools directory's Steering group.
...
`drowse/io/templates.py::_validate_body` / `_validate_context` one-for-one — slot
...
(export/import), `Save`/`LoadConversation` (the v7 whole-tree file),
...
`buildSamplingPayload`. `CastDrawer` owns those two labels through the Drowse
...
one-shot generation reuses such a leaf by forcing its text as the prefix — the
  general prefill mechanism. Explicit fan-out, regeneration, token forks and the
...
chips, per-layer strip, depth CoM), **logits** (ranked top-K alts with
  absolute-probability bars + captured-alternative forks), **sae**, **j-lens**
  (each: pinned
...
`ReplayReadout` (captured-envelope-preferred, keyed and deduplicated so pending
and completed results survive token navigation) and one `InstrumentHeader` provenance row —
...
`drowse.chat.v4.<model_id>`, a `{version: 4, model_id, saved_at, highlight}`
...
downloaded JSON file at **`version: 7`** carrying the complete loom tree
(`tree_format`, `drowse_version`, `root_id`, `active_node_id`, `rev`, `nodes`,
`children_of`, `cast`), the visual steer rack plus `subspaceAlong` (or an
authoritative custom full-grammar expression), the probe rack's exact attach
selectors, aliases, and nearest-node counts, highlight and sampling sections.
Every field is required and validated exactly — including a
...
The Python tree's `session_id` can be explicitly null even when the containing
dashboard snapshot names the HTTP session. Snapshot validation preserves that
value; non-null session IDs and model IDs must agree with the outer snapshot.
...
- **No `~`/`|` projection authoring in the rack.** Rank-one concepts, J-lens
  tokens, and SAE features expose push/ablate directly. The grammar
...
- **No pack-backed catalog plumbing in the drawers** — they read the live manifold
  catalog. There is no `/drowse/v1/packs*`, `/manifold-probes`, `/extract/preview`
