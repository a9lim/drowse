...
| `runners/__init__.py` | Barrel: re-exports every runner and helper as the `drowse.cli.runners.<name>` import/patch surface |

The barrel is load-bearing. Submodule runners call the session/config helpers
*through the package object* (`import drowse.cli.runners as _pkg`;
`_pkg._make_session(...)`), so `monkeypatch.setattr("drowse.cli.runners.<name>",
...
A bare `drowse`, or a verb group with no subverb, prints help and exits **0** —
...
`@_drowse_error_exit` wraps all eight top-level runners (and serve's two
live-readout policy helpers): an escaping `DrowseError` prints `user_message()`
...
without it a first `drowse manifold fit default/personas` would miss the bundled
folder.
...
point for every `-c`-taking subcommand. It composes `~/.drowse/config.yaml` plus
...
<target> [-a NS/NAME] [-f]` (HF coord `<ns>/<name>[@revision]`, local folder,
or local `.drowse`); `search [query] [-v] [-j]`; `push <selector> [-a OWNER/NAME] [-m]
...
`~/.drowse/manifolds/` and load no model (`install` and `search` hit HF).
...
`export archive` writes one exact manifold closure plus its referenced
template, when present, through `io.drowse_archive.export_drowse_archive`.
...
- `fetch <model> [source] [--revision REV] [--repo REPO] [-f] [-j]` — pure IO,
  ...
  stay in the Hugging Face cache; Drowse writes only the pinned binding.
...
- No `argv[0]` peeking and no bare-model fallback: `drowse google/gemma-2-2b-it`
