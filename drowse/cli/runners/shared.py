"""Shared helpers for the drowse CLI runners.
...
def _drowse_error_exit(fn: Callable[..., _R]) -> Callable[..., _R]:
    """Translate any ``DrowseError`` escaping a runner to a stderr line + exit.
...
def _print_verb_menu(group: str, verbs: "list[tuple[str, str]]") -> None:
    """Print the bare-verb-group menu every ``drowse <group>`` shows.

    ``group`` is the invocation path under ``drowse`` (``"pack"``,
    ...
    """
    print(f"usage: drowse {group} <verb> [...]")
    print()
    width = max(len(verb) for verb, _ in verbs)
    for verb, desc in verbs:
        print(f"  {verb:<{width}}  {desc}")
    print()
    print(f"Run `drowse {group} <verb> -h` for verb-specific options.")
...
def _make_session(args: argparse.Namespace, *, load_probes: bool = True):
    ...
    return DrowseSession.from_pretrained(
...
def _print_model_info(session: DrowseSession) -> None:
...
def _load_effective_config(
    ...
    """Compose ~/.drowse/config.yaml + any -c files and stamp args in place.
    ...
    ``experiment`` verbs.
    """
    from drowse.cli.config_file import (
...
def _setup_steering_vectors(
    ...
    *,
    ...
    """
    from drowse.io.selectors import canonicalize_atom, AmbiguousSelectorError
...
def _warmup_session(session: DrowseSession) -> None:
...
def _iter_manifold_folders(namespace: str | None):
    ...
    Thin wrapper over :func:`drowse.io.manifolds.iter_manifold_folders` —
    ...
    without this hop, a user's first ``drowse manifold fit
    ...
    from drowse.io.manifolds import iter_manifold_folders
...
def _resolve_manifold_ns_name(name: str) -> tuple[str, str]:
    ...
    # an existing ``~/.drowse`` that predates it — the ``ns/name`` branch below
