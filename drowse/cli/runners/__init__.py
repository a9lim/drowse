"""Runner functions for drowse CLI subcommands.
...
so the historical import surface ``drowse.cli.runners.<name>`` (and the
...
through this package object (``import drowse.cli.runners as _pkg``), so a
``monkeypatch.setattr("drowse.cli.runners.<name>", ...)`` still takes effect.
"""
...
# so ``drowse.cli.runners._load_or_fit_transfer_alignment`` still resolves and
...
from drowse.cli.runners.pack import (
    _PACK_RUNNERS,
    _run_pack,
    _run_pack_clear,
    _run_pack_export,
...
from drowse.cli.runners.template import (
    _TEMPLATE_RUNNERS,
...
from drowse.cli.runners.config import (
    _run_config,
    _run_config_show,
    _run_config_validate,
)
from drowse.cli.runners.lens import (
    _LENS_DOC_CHARS,
    _LENS_RUNNERS,
    _load_lens_corpus,
...
from drowse.cli.runners.sae import (
    _SAE_RUNNERS,
...
from drowse.cli.runners.experiment import (
    _EXPERIMENT_RUNNERS,
    _TRANSCRIPT_RUNNERS,
    _TRANSCRIPT_VERBS,
    _parse_grid_terms,
