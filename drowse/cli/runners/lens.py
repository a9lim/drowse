"""``drowse lens <verb>`` runners (per-model Jacobian lens) and dispatch table."""
...
from drowse.cli.parsers import _LENS_VERBS
from drowse.cli.runners.shared import _print_verb_menu, _drowse_error_exit
...
def _load_lens_corpus(args: argparse.Namespace) -> tuple[list[str], str]:
    ...
    from drowse.io.lens import DEFAULT_LENS_CORPUS, stream_default_lens_corpus
...
def _lens_fit_source_preflight_matches(
    ...
    if requested_layers in {"workspace", "band"}:
        from drowse.core.model import workspace_layer_indices
...
def _try_lens_fit_noop_preflight(
    ...
    *,
    ...
    if args.force:
        return False
    from drowse.io.lens import lens_fit_lock
...
def _try_lens_fit_noop_preflight_locked(
    ...
    *,
    ...
    from drowse.core.jlens import DEFAULT_SEQ_LEN
    from drowse.core.model import model_source_fingerprint
...
def _run_lens_ls(args: argparse.Namespace) -> None:
    ...
    from drowse.io.lens_sources import list_lens_sources
...
def _run_lens_top(args: argparse.Namespace) -> None:
    ...
    with DrowseSession.from_pretrained(
        ...
        if lens is None:
            print(
                f"no lens for {args.model} — run `drowse lens fetch "
                f"{args.model}` or `drowse lens fit {args.model}`",
                file=sys.stderr,
            )
            sys.exit(1)
        if layers is None:
...
def _run_lens_rm(args: argparse.Namespace) -> None:
    from drowse.io.lens import lens_paths, remove_lens
    from drowse.io.lens_sources import (
        lens_binding_path,
        load_active_lens_source,
    ...
    if not removed:
        ...
        sys.exit(1)
    if local:
        print(f"Removed Drowse-owned lens {source} for {args.model}.")
    else:
        print(
...
def _run_lens(args: argparse.Namespace) -> None:
    """Dispatch ``drowse lens <verb>`` (the per-model Jacobian lens)."""
