"""``drowse serve`` runner and its live-readout startup policy helpers."""
...
from drowse.cli.runners.shared import (
    _drowse_error_exit, _setup_steering_vectors, _warmup_session,
)
...
def _run_serve(args: argparse.Namespace) -> None:
    try:
        ...
        # fires when they've been uninstalled out from under drowse.
        print(
            "Server dependencies not installed. Run:\n"
            "  pip install --upgrade drowse.ai",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not args.model:
        print(
            ...
            "  drowse serve google/gemma-2-2b-it\n"
    ...
    # way for casual users to drive drowse; ``--no-web`` opts out for
