"""``drowse manifold <verb>`` compute runners (extract/generate/from-template/
...
import drowse.cli.runners as _pkg
from drowse.cli.parsers import _MANIFOLD_VERBS
from drowse.cli.runners.shared import (
    _print_verb_menu, _resolve_manifold_folder, _resolve_manifold_ns_name,
    _drowse_error_exit, _split_manifold_ns_name,
)
from drowse.io.paths import VARIANT_SUFFIX_RE
...
def _run_manifold_bake(args: argparse.Namespace) -> None:
    from drowse.io import bake as bake_mod
...
def _try_manifold_fit_noop_preflight(
    ...
    *,
    ...
    """
    from drowse.io.manifold_folder import ManifoldFolder, ManifoldFormatError
...
def _split_variant_suffix(raw: str) -> tuple[str, str | None]:
    ...
    grammar owned by :data:`drowse.io.paths.VARIANT_SUFFIX_RE`.
...
def _fold_all_fitted_manifolds(
    ...
    from drowse.io.manifold_tensors import load_manifold
    from drowse.core.capture import folded_directions
    from drowse.io.paths import manifolds_dir, tensor_filename
...
def _run_manifold_compare(args: argparse.Namespace) -> None:
    ...
    from drowse.io.selectors import parse as sel_parse, resolve
...
def _run_manifold_why(args: argparse.Namespace) -> None:
    ...
    if profile is None:
        print(
            ...
            f"`drowse manifold fit {name_part} -m {args.model}`.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def _print_why_histogram(
    ...
    import shutil
    from drowse.core.histogram import HIST_BUCKETS, bucketize
...
def _run_manifold_generate(args: argparse.Namespace) -> None:
    """``drowse manifold generate`` -- author + LLM-fill a discover folder (A2).

    Two-step end-to-end: load the model, run :meth:`DrowseSession.generate_responses`
    ...
    print(f"  -> run `drowse manifold fit {namespace}/{name}` to fit")


def _run_manifold_from_template(args: argparse.Namespace) -> None:
    """``drowse manifold from-template`` — author a discover manifold from a template.
    ...
    ``drowse manifold fit <name> -m MODEL``.
    """
    from drowse.io.manifolds import ManifoldFormatError, create_manifold_from_template
    ...
    print(f"  -> run `drowse manifold fit {namespace}/{name} -m MODEL` to fit")
...
def _run_manifold_merge(args: argparse.Namespace) -> None:
    ...
    print(f"Merged manifold written to {dst}")
    print(
        f"  -> run `drowse manifold fit {target_ns}/{target_name}` to fit"
    )
...
def _run_manifold_transfer(args: argparse.Namespace) -> None:
    ...
    dict to :func:`drowse.io.manifolds.transfer_manifold`, which is pure-io and
    ...
    from drowse.io.paths import encode_release_id, manifold_dir
...
def _run_manifold(args: argparse.Namespace) -> None:
    """Dispatch ``drowse manifold <verb>`` (the compute verbs)."""
