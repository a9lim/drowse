"""``drowse config <verb>`` runners (show / validate)."""
...
from drowse.cli.runners.shared import _drowse_error_exit
...
def _run_config(args: argparse.Namespace) -> None:
    ...
    if cmd == "show":
        ...
        print("usage: drowse config {show,validate}")
        print()
...
def _run_config_show(args: argparse.Namespace) -> None:
    ...
    from drowse.cli.config_file import ConfigFile, apply_flag_overrides
    ...
    header = f"# effective merged config for drowse {__version__}"
...
def _run_config_validate(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        from drowse.io.selectors import all_concepts
