"""``drowse pack <verb>`` lifecycle/distribution runners and dispatch table."""
...
from drowse.cli.parsers import _PACK_VERBS
...
def _run_pack_ls(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "json_output", False):
        ...
        return
    if not rows:
        print("no manifolds installed under ~/.drowse/manifolds/")
        return
...
def _run_pack_show(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "json_output", False):
        ...
        # server's ``GET /drowse/v1/manifolds/{ns}/{name}`` route so both
    ...
    if fitted:
        ...
        print("  fitted models: (none — run `drowse manifold fit`)")
...
def _run_pack_search(args: argparse.Namespace) -> None:
    import json as _json
    from drowse.io.hf_manifolds import search_manifolds
    ...
    try:
        ...
        print(f"drowse manifold search unavailable: {e}", file=sys.stderr)
        sys.exit(1)
...
def _run_pack_push(args: argparse.Namespace) -> None:
    from drowse.io.hf import resolve_target_coord
    from drowse.io.hf_manifolds import push_manifold
...
def _run_pack_clear(args: argparse.Namespace) -> None:
    from drowse.io.manifolds import clear_manifold_tensors
...
def _run_pack_refresh(args: argparse.Namespace) -> None:
    from drowse.io.manifolds import refresh_manifold
...
def _run_pack_export(args: argparse.Namespace) -> None:
    """Export a manifold to GGUF or a ``.drowse`` closure."""
    ...
    if fmt == "archive":
        ...
        try:
            written = export_drowse_archive(ns, name, output=args.output)
...
# The nine manifold *lifecycle* verbs (pure-IO over ~/.drowse/manifolds/,
...
def _run_pack(args: argparse.Namespace) -> None:
    """Dispatch ``drowse pack <verb>`` (the manifold lifecycle verbs)."""
