"""``drowse template <verb>`` runners (the templated-completion artifact)."""
...
from drowse.cli.parsers import _TEMPLATE_VERBS
from drowse.cli.runners.shared import (
    _print_verb_menu, _drowse_error_exit, _split_manifold_ns_name,
)
...
def _run_template_create(args: argparse.Namespace) -> None:
    """``drowse template create`` — author a standalone template artifact.
    ...
    invariants, and writes ``~/.drowse/templates/<ns>/<name>/template.json``.
    """
    import json as _json

    from drowse.io.templates import TemplateFormatError, create_template_folder
    ...
    print(f"  -> `drowse template score {namespace}/{name} -m MODEL` to read the "
...
def _run_template_ls(args: argparse.Namespace) -> None:
    ...
    from drowse.io.templates import iter_template_folders
    ...
    if getattr(args, "json_output", False):
        ...
        return
    if not rows:
        print("no templates installed under ~/.drowse/templates/")
        return
    for t in rows:
...
def _run_template_score(args: argparse.Namespace) -> None:
    """``drowse template score`` — the restricted-choice value distribution."""
    import json as _json

    from drowse.core.scoring import score_template
...
def _run_template(args: argparse.Namespace) -> None:
    """Dispatch ``drowse template <verb>`` (the templated-completion artifact)."""
