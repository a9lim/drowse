"""Argparse builders for the drowse CLI."""
...
# Verb table for ``drowse manifold <verb>`` — the *compute* surface.  A
...
# Verb table for ``drowse pack <verb>`` — the *lifecycle* surface (install /
...
_PACK_VERBS: list[tuple[str, str]] = [
    ...
    ("export",    "Export a manifold as GGUF or .drowse"),
]
...
# Verb table for ``drowse template <verb>`` — the standalone templated-completion
...
# Verb table for ``drowse lens <verb>`` — the per-model Jacobian lens
# (Gurnee et al. 2026): per-layer transport ``J_l = E[∂h_final/∂h_l]`` +
# vocabulary readout.  ``fit`` loads a model and runs the only backward
# passes in drowse; ``top``/``decompose`` load a model for one forward;
# lifecycle verbs are source-aware: local payloads live under DROWSE_HOME,
...
def _build_serve_parser(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("-k", "--api-key", default=None, metavar="KEY",
                        help="Require Bearer token auth; falls back to $DROWSE_API_KEY")
...
def _build_vector_extract(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument(
        ...
        help="Destination namespace for the extracted concept manifold. "
             "Unset lands under "
             "``~/.drowse/manifolds/local/<canonical>/``. Any other "
             "value relocates to "
...
def _build_vector_compare(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument(
        ...
        help=(
            ...
            "means under ~/.drowse/models/<id>/ to build the per-layer "
...
def _build_manifold_fit(fit: argparse.ArgumentParser) -> None:
    """``drowse manifold fit`` — fit an authored *or* discover-mode manifold.
...
def _build_pack_install(install: argparse.ArgumentParser) -> None:
    install.add_argument(
        "target",
        help="<ns>/<name>[@revision], manifold folder, or .drowse path",
    )
    install.add_argument(
...
def _build_pack_export(export: argparse.ArgumentParser) -> None:
    ...
    archive = export_fmt.add_parser(
        "archive",
        help="Export a manifold and referenced template as .drowse",
    )
    archive.add_argument("name", help="Manifold name (or ns/name)")
    archive.add_argument("-o", "--output", default=None, metavar="FILE")
...
_MANIFOLD_DESCRIPTIONS: dict[str, str] = {
    ...
    "generate": (
        ...
        "~/.drowse/manifolds/<ns>/<name>/ ready for `drowse manifold fit` "
    ...
    "from-template": (
        ...
        "template (`drowse template create`). Resolves the template, expands its "
        ...
        "load); run `drowse manifold fit <name> -m MODEL` next. fit_mode auto "
    ...
    "fit": (
        ...
        "folders (fit_mode pca/spectral, usually authored by `drowse manifold "
    ...
    "merge": (
        ...
        "a fresh, unfitted discover folder, then run `drowse manifold fit "
...
_PACK_DESCRIPTIONS: dict[str, str] = {
    "ls": "List local manifolds under ~/.drowse/manifolds/.",
    "show": (
        "Print one manifold's domain, node coordinates, and per-model fitted "
        "tensors."
    ),
    "install": (
        "Pull a manifold from a HuggingFace drowse-manifold repo "
        "(`<ns>/<name>[@revision]`) or copy-install a local folder path."
    ),
    "search": "Search HF for `drowse-manifold`-tagged model repos.",
    "push": (
        "Push a manifold folder (corpus + fitted tensors) to HF as a "
        "`drowse-manifold`-tagged model repo.  The corpus is always uploaded "
    ...
    "export": (
        "Export either a folded 2-node ``pca`` manifold as a llama.cpp GGUF, "
        "or one complete manifold closure as a validated ``.drowse``. The "
        "archive form preserves the manifold folder and includes its exact "
        "referenced template when present. Checksums establish integrity, not "
        "publisher identity."
    ),
}
...
_TEMPLATE_DESCRIPTIONS: dict[str, str] = {
    "create": (
        ...
        "~/.drowse/templates/<ns>/<name>/. A template is a slot token, a set of "
    ...
    "ls": "List local templates under ~/.drowse/templates/.",
...
def _build_lens_fit(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument(
        ...
        help="Text corpus: one document per line (a JSONL line with a 'text' "
             ...
             "via the `datasets` library (pip install 'drowse.ai[hf]').",
    )
    p.add_argument(
...
_LENS_DESCRIPTIONS: dict[str, str] = {
    "fit": (
        ...
        "backward passes per document — the only backward passes in drowse. "
        "The artifact lands under ~/.drowse/models/<model>/jlens/local/default "
    ...
    "fetch": (
        ...
        "small Drowse binding. No provider payload is copied into DROWSE_HOME."
    ),
    "ls": "List Drowse-owned local lenses and external cache bindings.",
...
def _build_manifold_parser(parser: argparse.ArgumentParser) -> None:
    """``drowse manifold`` — the steering-vector / manifold compute verbs."""
...
def _build_pack_parser(parser: argparse.ArgumentParser) -> None:
    """``drowse pack`` — the manifold lifecycle (install/share/inspect) verbs."""
...
def _build_template_parser(parser: argparse.ArgumentParser) -> None:
    """``drowse template`` — the standalone templated-completion artifact."""
...
def _build_lens_parser(parser: argparse.ArgumentParser) -> None:
    """``drowse lens`` — the per-model Jacobian-lens artifact."""
...
def _build_sae_parser(parser: argparse.ArgumentParser) -> None:
    """``drowse sae`` — local/external sparse-autoencoder sources."""
    ...
    for verb, desc in _SAE_VERBS:
        ...
        if verb == "train":
            ...
            # the Hugging Face cache and Drowse writes a small binding.  The
...
def _build_experiment_transcript_parser(parser: argparse.ArgumentParser) -> None:
    """``drowse experiment transcript`` — replay saved tree paths."""
    ...
    run.add_argument("path", help="Path to a drowse_transcript YAML file")
...
def _build_experiment_naturalness(p: argparse.ArgumentParser) -> None:
    """``drowse experiment naturalness`` — behavior-manifold naturalness eval.
...
def _build_root_parser() -> argparse.ArgumentParser:
    ...
    cfg = sub.add_parser(
        "config",
        help="Inspect and validate drowse config files",
