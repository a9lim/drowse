"""Plotly figure builders for drowse data structures.
...
Plotly is imported lazily — bare ``import drowse.notebook`` succeeds even
...
if TYPE_CHECKING:
    ...
    from drowse.core.results import ProbeReadings, ResultCollector


class NotebookExtraNotInstalled(ImportError, DrowseError):
    ...
    def __init__(self, missing: str) -> None:
        super().__init__(
            f"drowse.notebook needs '{missing}' but it is not installed. "
            f"Install the extra: pip install 'drowse.ai[notebook]'"
        )
...
def plot_alpha_sweep(
    ...
    *,
    ...
    ``source`` accepts the same shapes as :func:`drowse.notebook.to_dataframe`.
...
def plot_probe_correlation(
    ...
    *,
    ...
    :class:`~drowse.core.mahalanobis.LayerWhitener` covering the profiles'
...
def plot_layer_norms(
    ...
    *,
    ...
    notebook-side counterpart to the ``drowse manifold why`` output, surfaced
