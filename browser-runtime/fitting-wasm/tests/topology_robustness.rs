...
fn samples(fields: &[&str]) -> (Vec<Vec<f64>>, Vec<Vec<f64>>) {
    ...
    for i in 0..n {
        ...
        for j in 0..m {
            ...
            let z = if m > 1 {
                width * (2.0 * j as f64 / (m - 1) as f64 - 1.0)
            } else {
                0.0
            };
            let mut point = match shape {
                ...
                "donut" => vec![
                    (1.0 + width * v.cos()) * u.cos(),
                    (1.0 + width * v.cos()) * u.sin(),
                    width * v.sin(),
                ],
                "mobius" => vec![
                    (1.0 + z * (u / 2.0).cos()) * u.cos(),
                    (1.0 + z * (u / 2.0).cos()) * u.sin(),
                    z * (u / 2.0).sin(),
                ],
            ...
            for (axis, x) in point.iter_mut().enumerate() {
                *x = f64::from(
                    (*x + noise * ((i * m + j) as f64 * 12.9898 + axis as f64 * 78.233).sin())
                        as f32,
                );
            }
            points.push(point);
            truth.push(if shape == "product" {
                vec![u, v]
            } else {
                vec![u]
            });
...
fn gram(points: &[Vec<f64>]) -> Vec<f64> {
    ...
    let means: Vec<f64> = (0..points[0].len())
        .map(|axis| points.iter().map(|row| row[axis]).sum::<f64>() / n as f64)
        .collect();
    points
        .iter()
        .flat_map(|left| {
            points.iter().map(|right| {
                left.iter()
                    .zip(right)
                    .zip(&means)
                    .map(|((a, b), mean)| (a - mean) * (b - mean))
                    .sum()
            })
        })
        .collect()
...
fn shared_adversarial_shapes_and_coordinate_fidelity() {
    for line in include_str!("../../fixtures/topology-adversarial-v1.csv")
        .lines()
        .skip(1)
    {
        ...
        for variant in 0..3 {
            ...
            let chart = match result {
                ...
                Err(error) => {
                    assert!(
                        error.to_string().contains("connected components"),
                        "{line}: {error}"
                    );
                    None
                }
            ...
            assert_eq!(
                chart.as_ref().map_or(0, |chart| chart.dimensions),
                expected,
            ...
            if let Some(chart) = chart {
                ...
                for axis in 0..chart.dimensions {
                    ...
                    for (target, _) in truth[0].iter().enumerate() {
                        for sign in [-1.0, 1.0] {
                            let phases: Vec<f64> = (0..points.len())
                                .map(|row| {
                                    chart.angles[row * chart.dimensions + axis]
                                        + sign * truth[row][target]
                                })
                                .collect();
                            let re =
                                phases.iter().map(|x| x.cos()).sum::<f64>() / points.len() as f64;
                            let im =
                                phases.iter().map(|x| x.sin()).sum::<f64>() / points.len() as f64;
                    ...
                    assert!(
                        best.1 > 0.95,
                        "folded/mixed axis: {line}, variant {variant}, coherence {}",
                        best.1
                    );
                    assert!(
                        !matches.contains(&best.0),
                        "duplicate/harmonic axis: {line}"
                    );
                    matches.push(best.0);
                }
            }
